...
{
    if (DEBUG > 2)
	log_info("called %s with %s, %s, %d", __PRETTY_FUNCTION__, SAFE_NAME(keeper),
		 SAFE_NAME(ch), shop_nr);
...
void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    if (DEBUG > 2)
	log_info("called %s with %s, %s, %s, %d", __PRETTY_FUNCTION__, VNULL(arg),
		 SAFE_NAME(ch), SAFE_NAME(keeper), shop_nr);
 ...
 */
    ...
    sprintf(buf, shop_index[shop_nr].message_sell,
	    GET_NAME(ch), (int)(temp1->obj_flags.cost * shop_index[shop_nr].profit_sell));
