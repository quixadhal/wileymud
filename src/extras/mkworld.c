...
struct commands_st
{
    const char *name;
    int num;
} commands[] = {{"brief", BRIEF},   {"change", CHANGE}, {"copy", COPY},     {"down", DOWN},   {"desc", DESC},
                {"delete", DELETE}, {"dig", DIG},       {"east", EAST},     {"exits", EXITS}, {"extra", EXTRA},
                {"flag", FLAG},     {"format", FORMAT}, {"goto", GOTO},     {"help", HELP},   {"look", LOOK},
                {"light", LIGHT},   {"link", LINK},     {"list", LIST},     {"make", MAKE},   {"map", MAP},
                {"north", NORTH},   {"quit", QUIT},     {"south", SOUTH},   {"save", SAVE},   {"sector", SECTOR},
                {"title", TITLE},   {"up", UP},         {"unlink", UNLINK}, {"west", WEST},   {"zone", ZONE},
                {NULL, -1}};
...
struct dir_st
{
    ...
    int num;
                  ...
                  {"up", 4},    {"down", 5}, {"No Where", -1}, {NULL, -1}};

const char *rev_text[] = {"south", "west", "north", "east", "down", "up"};
...
struct room_flg_st
{
    const char *name;
    int mask;
} room_flags[] = {{"Dark", 1},     {"Death", 2},     {"Nomob", 4},     {"Indoor", 8},
                  {"Peace", 16},   {"Nosteal", 32},  {"Nosummon", 64}, {"Nomagic", 128},
                  {"Tunnel", 256}, {"Private", 512}, {"Sound", 1024},  {NULL, -1}};

struct door_st
{
    const char *name;
    int num;
} door_states[] = {{"Open Passage", 0},
...
struct sector_st
{
    const char *name;
    int num;
} sector_types[] = {{"Inside", 0},   {"City", 1},  {"Field", 2}, {"Forest", 3},     {"Hill", 4},      {"Mountain", 5},
                    {"Swimming", 6}, {"River", 7}, {"Air", 8},   {"Underwater", 9}, {"Teleport", -1}, {NULL, -1}};
...
void bug_logger(char *File, char *Func, int Line, int Verbose, const char *Str, ...)
{
    ...
    if (File || Func || Line)
    {
        ...
        sprintf(Time, "%02d.%02d.%02d.%02d.%02d.%02d.%03d", now_part->tm_year, now_part->tm_mon, now_part->tm_mday,
...
void Load_Rooms(void)
{
    ...
    while (fgets(tmp_str, BUFFER_SIZE - 1, fp))
    {
        ...
        if (fgets(tmp_str, BUFFER_SIZE - 1, fp) == NULL)
        {
            log_info("Room %d: Unexpected EOF reading flags for room [#%d].", number_rooms, current->room_id);
            exit(ERR_EOF);
        }
        idx = sscanf(tmp_str, "%d %d %d", &current->zonenum, &current->room_flag, &current->sector_type);
        if (idx < 3)
        {
            trunc_string(tmp_str);
            log_info("Room %d: Expected zone, flags and sector type for [#%d],\nFormat should have been \"%%d %%d "
                     "%%d\", Got this instead:\n%s\n",
                     number_rooms, current->room_id, tmp_str);
            exit(ERR_ROOMFLAGS);
        }
        ...
        if (current->sector_type == -1)
        {
            ...
            idx = sscanf(tmp_str, "%d %d %d %d %d %d %d", &current->zonenum, &current->room_flag, &current->sector_type,
                         &current->teleport->time, &current->teleport->to_room, &current->teleport->do_look,
                         &current->teleport->sector_type);
            if (idx < 7)
            {
                trunc_string(tmp_str);
                log_info("Room %d: Invalid teleport flags in teleport room [#%d],\nExpected format \"%%d %%d -1 %%d "
                         "%%d %%d %%d\", Got this:\n%s\n",
                         number_rooms, current->room_id, tmp_str);
                exit(ERR_TELEPORT);
            }
        ...
        else if (current->sector_type == 7)
        {
            ...
            idx = sscanf(tmp_str, "%d %d %d %d %d", &current->zonenum, &current->room_flag, &current->sector_type,
                         &current->river->speed, &current->river->direction);
            if (idx < 5)
            {
                trunc_string(tmp_str);
                log_info("Room %d: River room [#%d] is missing parameters,\nExpected format \"%%d %%d 7 %%d %%d\", Got "
                         "this:\n%s\n",
                         number_rooms, current->room_id, tmp_str);
                exit(ERR_RIVER);
            }
        ...
        while (!done)
        {
            if (fgets(tmp_str, BUFFER_SIZE - 1, fp) == NULL)
            {
                log_info("Room %d: Unexpected EOF reading exits for room [#%d].", number_rooms, current->room_id);
                exit(ERR_EOF);
            }
            switch (tmp_str[0])
            {
            case 'D':
                ...
                if (fgets(tmp_str, BUFFER_SIZE - 1, fp) == NULL)
                {
                    log_info("Room %d: Unexpected EOF reading exit keywords for room [#%d].", number_rooms,
                             current->room_id);
                    exit(ERR_EOF);
                }
                trunc_string(tmp_str);
                ...
                {
                    log_info("Room %d: Bad exit keywords in room [#%d]. Expected format \"%%s~\", Got:\n%s\n",
                             number_rooms, current->room_id, tmp_str);
                    exit(ERR_EXITKEY);
                }
                ...
                if (fgets(tmp_str, BUFFER_SIZE - 1, fp) == NULL)
                {
                    log_info("Room %d: Unexpected EOF reading exit data for room [#%d].", number_rooms,
                             current->room_id);
                    exit(ERR_EOF);
                }
                idx2 = sscanf(tmp_str, "%d %d %d", &current->exit[idx].door_flag, &current->exit[idx].key_number,
            ...
            case 'E':
                if (fgets(tmp_str, BUFFER_SIZE - 1, fp) == NULL)
                {
                    log_info("Room %d: Unexpected EOF reading extra keywords for room [#%d].", number_rooms,
                             current->room_id);
                    exit(ERR_EOF);
                }
                trunc_string(tmp_str);
                ...
                if (idx2)
                    current->extra = (Extra_t *)realloc(current->extra, (idx2 + 1) * sizeof(struct Extra_st));
                else
        ...
        for (idx2 = 0; idx2 < 6; idx2++)
            if (ptr->exit[idx2].door_flag > -1)
            {
                ...
                if (ptr->exit[idx2].real_to_room == NULL && ptr->exit[idx2].to_room != -1)
                {
                    ...
                    fprintf(errfile, "Room [#%d] -- Unable to resolve %s exit to room [#%d]\n", ptr->room_id,
                            directions[idx2].name, ptr->exit[idx2].to_room);
                }
            }
        ptr = ptr->next;
    }
...
void Main_Loop(void)
{
    ...
    do
    {
        ...
        printf("( Zone: %d Room: %d Terrain: %-s ) ", current->zonenum, current->room_id, sector_types[idx].name);
...
void Command_exits(void)
{
    ...
    for (idx = 0; idx < 6; idx++)
    {
        ...
        if (current->exit[idx].real_to_room != NULL)
        {
            printf("%-6s - %-s to Room %d (%-s)\n", directions[idx].name, door_states[idx2].name,
...
void Command_extra(void)
{
    ...
    if (find_keyword(inputs[1]) == DELETE)
    {
        ...
        current->extra = (Extra_t *)realloc(current->extra, (current->number_extras) * sizeof(struct Extra_st));
...
void Command_flag(void)
{
    ...
    if (inputs[1][0] == '\0' || room_flags[idx].name[0] == '\0' || (inputs[2][0] != '\0' && inputs[3][0] == '\0'))
    {
...
void Command_look(void)
{
    ...
    else
    {
        ...
        {
            idx = 0;
            while (idx < current->number_extras && strstr(current->extra[idx].keywords, inputs[1]) == 0)
                idx++;
...
void Command_save(void)
{
    ...
    while (ptr != NULL)
    {
        ...
        if (ptr->sector_type == -1)
            fprintf(out, " %d %d %d %d", ptr->teleport->time, ptr->teleport->to_room, ptr->teleport->do_look,
...
void Command_sector(void)
{
    ...
    if (sector_types[idx].name != NULL)
    {
        ...
        if (idx2 == -1)
        {
            if (inputs[2][0] == '\0' || inputs[3][0] == '\0' || inputs[4][0] == '\0' || inputs[5][0] == '\0')
            {
            ...
            while (sector_types[idx3].name != NULL && strcasecmp(sector_types[idx3].name, inputs[5]) != 0)
                idx3++;
