...
void do_hit(struct char_data *ch, const char *argument, int cmd)
{
	...
	if (victim) {
		...
		act("$N is your master! , you simply can't hit $M.", FALSE, ch, 0, victim,
		    TO_CHAR);
		return;
	    }
...
void do_kill(struct char_data *ch, const char *argument, int cmd)
{
	...
	if (!(victim = get_char_room_vis(ch, arg)))
	    ...
	    act("You hack $M to pieces! Blood flies everywhere!", FALSE, ch, 0, victim,
		TO_CHAR);
	    act("$N hacks you into itty bitty pieces!", FALSE, victim, 0, ch, TO_CHAR);
	    act("$n brutally hacks $N into itty bitty pieces", FALSE, ch, 0, victim,
		TO_NOTVICT);
	    if (RIDDEN(victim)) {
...
/* Buggy as hell... this should be redone. */
	...
	if (is_abbrev("kill", action) ||
	    ...
	    is_abbrev("punch", action) ||
	    is_abbrev("disarm", action) || is_abbrev("steal", action)
...
void do_flee(struct char_data *ch, const char *argument, int cmd)
{
	...
	if (CAN_GO(ch, attempt) &&
		...
		if (IS_NPC(ch)) {
		    ...
		    percent_chance =
			(int)100              *(float)GET_HIT(ch->specials.fighting) /
...
void do_bandage(struct char_data *ch, const char *argument, int cmd)
{
	...
	GET_MANA(ch) -= cost / 2;
	act("$N is trying to save you, bandages have been put on your wounds!", FALSE, victim,
	    0, ch, TO_CHAR);
    } else {
	if (GET_HIT(victim) <= 0)
	    GET_HIT(victim) = 1;
...
void slam_into_wall(struct char_data *ch, struct room_direction_data *exitp)
{
    ...
    if (DEBUG > 2)
	log_info("called %s with %s, %08zx", __PRETTY_FUNCTION__, SAFE_NAME(ch),
		 (size_t) exitp);
