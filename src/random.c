...
void random_death_message(struct char_data *ch, struct char_data *victim)
{
    ...
    const char *bugger[] = {"%s shouts '%s is no longer a problem.'\r\n",
                            ...
                            NULL};
    const char *mybugger[] = {"You shout '%s is no longer a problem.'\r\n",
    ...
    for (xx = descriptor_list; xx; xx = xx->next)
        if (xx->character != ch && !xx->connected && !IS_SET(xx->character->specials.new_act, NEW_PLR_NOSHOUT) &&
...
{
    static const char *oops[] = {"You just can't seem to concentrate on %s...\r\n",
...
{
    static const char *oops[] = {"Bylle Grylle Grop Gryf???",
