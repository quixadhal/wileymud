...
void MakeNoise(int room, char *local_snd, char *distant_snd)
{
    ...
    if (rp)
    {
        ...
        {
            if (!IS_NPC(ch) && local_snd && !IS_SET(ch->specials.act, PLR_DEAF) && (GET_POS(ch) != POSITION_SLEEPING))
...
void MakeSound(int current_pulse)
{
    ...
    for (obj = object_list; obj; obj = obj->next)
    {
        ...
        {
            if (((obj->obj_flags.value[0]) && (current_pulse % obj->obj_flags.value[0]) == 0) || (!number(0, 5)))
            {
                if (obj->carried_by)
                {
    ...
    {
        if ((IS_PC(ch)) && (number(0, 8) == 0) && (IS_NOT_SET(ch->specials.act, PLR_DEAF)) &&
