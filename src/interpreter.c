...
const char                             *command[] = {
    ...
    "echo", "stand", "sit", "rest", "sleep",
    "wake", "force", "transfer", "hug", "snuggle",
    "cuddle", "nuzzle", "cry", "news", "equipment",
    "buy", "sell", "value", "list", "drop",
    "goto", "weather", "read", "pour", "grab",
    "remove", "put", "shutdow", "save", "hit",
    "string", "give", "quit", "stat", "guard",
    "time", "load", "purge", "shutdown", "idea",
    "typo", "bug", "whisper", "cast", "at",
    "ask", "order", "sip", "taste", "snoop",
    "follow", "rent", "offer", "poke", "advance",
    "accuse", "grin", "bow", "open", "close",
    "lock", "unlock", "leave", "applaud", "blush",
    "burp", "chuckle", "clap", "cough", "curtsey",
    "fart", "flip", "fondle", "frown", "gasp",
    "glare", "groan", "grope", "hiccup", "lick",
    "love", "moan", "nibble", "pout", "purr",
    "ruffle", "shiver", "shrug", "sing", "slap",
    "smirk", "snap", "sneeze", "snicker", "sniff",
    "snore", "spit", "squeeze", "stare", "strut",
    "thank", "twiddle", "wave", "whistle", "wiggle",
    "wink", "yawn", "snowball", "write", "hold",
    ...
    "peer", "point", "ponder", "punch", "snarl",
    "spank", "steam", "tackle", "taunt", "think",
    "whine", "worship", "yodel", "brief", "wizlist",
    "consider", "group", "restore", "return", "switch",
...
int search_block(const char *arg, const char **list, char exact)
{
    ...
    if (DEBUG > 2)
	log_info("called %s with %s, %08zx, %d", __PRETTY_FUNCTION__, VNULL(arg), (size_t) list,
		 (int)exact);
...
int old_search_block(const char *argument, int begin, int arglen, const char **list, int mode)
{
    ...
    if (DEBUG > 2)
	log_info("called %s with %s, %d, %08zx, %d", __PRETTY_FUNCTION__, VNULL(argument),
		 arglen, (size_t) list, mode);
...
/* DO ALL NORMAL PARSING ABOVE HERE! */
	...
	log_error("command(%s), argument(%s), cmd(%d)\r\n", (hack + begin),
		  (hack + begin + look_at + got_args), cmd);
    }
    random_error_message(ch);
}
...
{
    if (DEBUG > 2)
	log_info("called %s with %s, %08zx", __PRETTY_FUNCTION__, VNULL(argument),
		 (size_t) dest);
...
{
    if (DEBUG > 2)
	log_info("called %s with %s, %08zx, %08zx", __PRETTY_FUNCTION__, VNULL(string),
...
void nanny(struct descriptor_data *d, char *arg)
{
	...
	case CON_GET_NAME:
		...
		dcprintf(d,
			 "\r\nWileyMUD is currently in registration-only mode.\r\nPlease email quixadhal@wileymud.themud.org for a character!\r\n");
		STATE(d) = CON_WIZLOCK;
		return;
	    }
...
/*  GET_NAME(d->character) = (char *)strdup(d->usr_name); */
		...
		if (already_mob_name(d->usr_name)) {
		    dcprintf(d,
			     "\rBut you'd be confused with a MONSTER!.\r\nWHAT is your Name? ");
		    return;
		}
		if (banned_name(d->usr_name)) {
		    dcprintf(d,
			     "\rSorry, that is a STUPID name... Try another.\r\nWHAT is your Name? ");
		    return;
		}
		GET_NAME(d->character) = (char *)strdup(d->usr_name);
		 ...
		 * log_info("New player!"); 
		 */
		log_auth(d->character, "NEW PLAYER %s (%s@%s/%s)!", GET_NAME(d->character),
			 d->username, host_name, ip_addr);
	    }
	    return;
	...
	case CON_MENU_SELECT:
		...
		case '1':
			...
			if ((strcasecmp(GET_NAME(d->character), "Quixadhal"))) {
			    ...
			    cprintf(d->character,
				    "Fool!  You DARE challenge the Dread Lord?\r\n");
		...
		case '5':
			...
			for (dd = descriptor_list; dd; dd = dd->next)
				...
				if (!IS_IMMORTAL(person) || IS_IMMORTAL(d->character)) {
				    lcount++;
				    dcprintf(d, "%s %s %s\r\n",
					     (person->player.pre_title ? person->
					      player.pre_title : "")
...
int ValidPlayer(char *who, char *pwd, char *oldpwd)
{
    ...
    if (DEBUG > 2)
	log_info("called %s with %s, %s, %s", __PRETTY_FUNCTION__, VNULL(who), VNULL(pwd),
		 VNULL(oldpwd));
