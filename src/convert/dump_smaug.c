...
const int smaug_convertsectors[] = { SMAUG_SECT_INDOORS,
                               SMAUG_SECT_INDOORS,
                               SMAUG_SECT_CITY,
                               SMAUG_SECT_FIELD,
                               SMAUG_SECT_FOREST,
...
/* ZONE */
    ...
    /* SMAUG apparently doesn't have reset modes */
    /* fprintf(ofp, "Mode\t%d\n", Zones->Zone[i].Mode); */
...
/* MOBS */
    ...
    for(j= 0; j< Mobs->Count; j++) {
      if((remap_zone_vnum(Zones, Mobs->Mob[j].Zone) == i) ||
        ...
        fprintf(ofp, "%s~\n", strip_cr(ext_flag_string(&MobValue, smaug_act_flags)));
        MobValue = smaug_convert_aflags(Mobs->Mob[j].AffectedBy);
        fprintf(ofp, "%s~\n", strip_cr(ext_flag_string(&MobValue, smaug_a_flags)));
	...
	/* might convert to race later... */
        fprintf(ofp, "%s~\n", flag_string( 0, smaug_lang_names ) );
        fprintf(ofp, "%s~\n", flag_string( 0, smaug_lang_names ) );
	...
	fprintf(ofp, "%s~\n", smaug_npc_race[get_npc_race(race_name(Mobs->Mob[j].Race))]);
	fprintf(ofp, "%s~\n", smaug_npc_class[get_npc_class(class_name(Mobs->Mob[j].Class))]);
        /* Wiley doesn't have limbs */
	/* fprintf(ofp, "%s~\n", flag_string( pMobIndex->xflags, part_flags) ); */
	fprintf(ofp, "%s~\n", flag_string( 0, smaug_part_flags) );

        ImmValue = smaug_convert_imm(Mobs->Mob[j].Resistance);
	fprintf(ofp, "%s~\n", ext_flag_string( &ImmValue, smaug_ris_flags) ); // resist
        ImmValue = smaug_convert_imm(Mobs->Mob[j].Immunity);
	fprintf(ofp, "%s~\n", ext_flag_string( &ImmValue, smaug_ris_flags) ); // immune
        ImmValue = smaug_convert_imm(Mobs->Mob[j].Susceptible);
	fprintf(ofp, "%s~\n", ext_flag_string( &ImmValue, smaug_ris_flags) ); // susc
        ImmValue = smaug_convert_imm(0);
	fprintf(ofp, "%s~\n", ext_flag_string( &ImmValue, smaug_ris_flags) ); // absorb -- Wiley has none

	//fprintf(ofp, "%s~\n", ext_flag_string(&pMobIndex->attacks, attack_flags) );
	//fprintf(ofp, "%s~\n", ext_flag_string(&pMobIndex->defenses, defense_flags) );
        ImmValue = smaug_convert_imm(0);
	fprintf(ofp, "%s~\n", ext_flag_string(&ImmValue, smaug_attack_flags) );
        ImmValue = smaug_convert_imm(0);
	fprintf(ofp, "%s~\n", ext_flag_string(&ImmValue, smaug_defense_flags) );
...
/* OBJECTS */
    ...
    for(j= 0; j< Objects->Count; j++) {
      if((remap_zone_vnum(Zones, Objects->Object[j].Zone) == i) ||
        ...
        fprintf(ofp, "%s~\n", strip_cr(smaug_o_types[smaug_convertobjtype(Objects->Object[j].Type)]));
        ObjValue = smaug_convert_oflags(Objects->Object[j].Flags.Extra);
        fprintf(ofp, "%s~\n", strip_cr(ext_flag_string(&ObjValue, smaug_o_flags)));
        WearValue = smaug_convert_wflags(Objects->Object[j].Flags.Wear);
	fprintf(ofp, "%s~\n", strip_cr(flag_string(WearValue, smaug_w_flags)));
        /* magic flags? Wiley don't need to steenkin magic flags! */
        fprintf(ofp, "%s~\n", strip_cr(flag_string(0, smaug_mag_flags)));
...
/*
	...
	fprintf( fpout, "%s~\n", flag_string(pObjIndex->weasmaug_r_flags, w_flags) );
    ...
    for(j= 0; j< Rooms->Count; j++) {
      if((remap_zone_vnum(Zones, Rooms->Room[j].Zone) == i) ||
        ...
        if ( Rooms->Room[j].Sector == SECT_TELEPORT ) {
          fprintf(ofp, "%s~\n", strip_cr(smaug_sect_types[smaug_convertsector[Rooms->Room[j].TeleportSector]]));
        } else {
          fprintf(ofp, "%s~\n", strip_cr(smaug_sect_types[smaug_convertsector[Rooms->Room[j].Sector]]));
        }

        /* First, convert bits */
        RoomValue = smaug_convert_rflags(Rooms->Room[j].Flags);
        fprintf(ofp, "%s~\n", strip_cr(ext_flag_string(&RoomValue, smaug_r_flags)));
        ...
        for(k= 0; k< Rooms->Room[j].ExitCount; k++) {
          switch(Rooms->Room[j].Exit[k].Error) {
            ...
            case EXIT_ONE_WAY:
              ...
              */
              fprintf( ofp, "%s", "D\n" );
              fprintf( ofp, "%s~\n", strip_cr(smaug_dir_name[Rooms->Room[j].Exit[k].Direction] ));
              ...
              fprintf( ofp, "%s~\n", ext_flag_string(&ExitValue, smaug_ex_flags) );
...
*/
  ...
  qsort(ZoneSort, Zones->Count, sizeof(pair), smaug_qcmp_zone_vnum);
  sprintf(filename, "%s/%s/area.lst", OutputDir, SMAUG_SUBDIR);
  listfp= open_file(filename, "w");
  for(i= 0; i< Zones->Count; i++) {
    if(!Quiet)
