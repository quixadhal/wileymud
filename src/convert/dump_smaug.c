...
const int smaug_convertsectors[] = {SMAUG_SECT_INSIDE,   SMAUG_SECT_INSIDE,     SMAUG_SECT_CITY,
                                    SMAUG_SECT_FIELD,    SMAUG_SECT_FOREST,     SMAUG_SECT_HILLS,
                                    SMAUG_SECT_MOUNTAIN, SMAUG_SECT_WATER_SWIM, SMAUG_SECT_WATER_NOSWIM,
                                    SMAUG_SECT_AIR,      SMAUG_SECT_UNDERWATER};
...
char *const smaug_dir_name[] = {"north",     "east",      "south",     "west",      "up",       "down",
                                "northeast", "northwest", "southeast", "southwest", "somewhere"};

char *const smaug_area_flags[] = {"nopkill", "freekill", "noteleport", "spelllimit", "prototype", "r5",  "r6",  "r7",
                                  "r8",      "r9",       "r10",        "r11",        "r12",       "r13", "r14", "r15",
                                  "r16",     "r17",      "r18",        "r19",        "r20",       "r21", "r22", "r23",
                                  "r24",     "r25",      "r26",        "r27",        "r28",       "r29", "r30", "r31"};

char *const smaug_ex_flags[] = {"isdoor",    "closed", "locked",   "secret",     "swim",       "pickproof",
                                "fly",       "climb",  "dig",      "eatkey",     "nopassdoor", "hidden",
                                "passage",   "portal", "r1",       "r2",         "can_climb",  "can_enter",
                                "can_leave", "auto",   "noflee",   "searchable", "bashed",     "bashproof",
                                "nomob",     "window", "can_look", "isbolt",     "bolted"};

char *const smaug_r_flags[] = {"dark",      "death",         "nomob",    "indoors",   "lawful",   "neutral",
                               "chaotic",   "nomagic",       "tunnel",   "private",   "safe",     "solitary",
                               "petshop",   "norecall",      "donation", "nodropall", "silence",  "logspeech",
                               "nodrop",    "clanstoreroom", "nosummon", "noastral",  "teleport", "teleshowdesc",
                               "nofloor",   "nosupplicate",  "arena",    "nomissile", "r4",       "r5",
                               "prototype", "dnd",           "bfs_mark"};
 ...
 */
char *const smaug_sect_types[] = {"inside",       "city",       "field", "forest", "hills", "mountain",   "water_swim",
                                  "water_noswim", "underwater", "air",   "desert", "dunno", "oceanfloor", "underground",
                                  "lava",         "swamp",      "r1",    "r2",     "r3",    "r4",         "r5",
                                  "r6",           "r7",         "r8",    "r9",     "r10",   "r11",        "r12",
                                  "r13",          "r14",        "r15",   "r16"};

char *const smaug_o_types[] = {
    "none",          "light",      "scroll",    "wand",    "staff",      "weapon", "_fireweapon", "_missile",
    "treasure",      "armor",      "potion",    "_worn",   "furniture",  "trash",  "_oldtrap",    "container",
    "_note",         "drinkcon",   "key",       "food",    "money",      "pen",    "boat",        "corpse",
    "corpse_pc",     "fountain",   "pill",      "blood",   "bloodstain", "scraps", "pipe",        "herbcon",
    "herb",          "incense",    "fire",      "book",    "switch",     "lever",  "pullchain",   "button",
    "dial",          "rune",       "runepouch", "match",   "trap",       "map",    "portal",      "paper",
    "tinder",        "lockpick",   "spike",     "disease", "oil",        "fuel",   "_empty1",     "_empty2",
    "missileweapon", "projectile", "quiver",    "shovel",  "salve",      "cook",   "keyring",     "odor",
    "chance",        "mix"};

char *const smaug_o_flags[] = {
    "glow",        "hum",        "dark",     "loyal",        "evil",     "invis",      "magic",      "nodrop",
    "bless",       "antigood",   "antievil", "antineutral",  "noremove", "inventory",  "antimage",   "antithief",
    "antiwarrior", "anticleric", "organic",  "metal",        "donation", "clanobject", "clancorpse", "antivampire",
    "antidruid",   "hidden",     "poisoned", "covering",     "deathrot", "buried",     "prototype",  "nolocate",
    "groundrot",   "lootable",   "personal", "multi_invoke", "enchanted"};

char *const smaug_w_flags[] = {"take", "finger", "neck",    "body",  "head",  "legs",  "feet", "hands",
                               "arms", "shield", "about",   "waist", "wrist", "wield", "hold", "_dual_",
                               "ears", "eyes",   "missile", "back",  "face",  "ankle", "r4",   "r5",
                               "r6",   "r7",     "r8",      "r9",    "r10",   "r11",   "r12",  "r13"};

char *const smaug_mag_flags[] = {"returning", "backstabber", "bane", "loyal", "haste", "drain", "lightning_blade",
...
char *const smaug_act_flags[] = {
    "npc",      "sentinel",   "scavenger", "r1",       "r2",       "aggressive", "stayarea",  "wimpy",
    "pet",      "train",      "practice",  "immortal", "deadly",   "polyself",   "meta_aggr", "guardian",
    "running",  "nowander",   "mountable", "mounted",  "scholar",  "secretive",  "hardhat",   "mobinvis",
    "noassist", "autonomous", "pacifist",  "noattack", "annoying", "statshield", "prototype"};

char *const smaug_a_flags[] = {
    "blind",   "invisible",   "detect_evil",    "detect_invis", "detect_magic", "detect_hidden",
    "hold",    "sanctuary",   "faerie_fire",    "infrared",     "curse",        "_flaming",
    "poison",  "protect",     "_paralysis",     "sneak",        "hide",         "sleep",
    "charm",   "flying",      "pass_door",      "floating",     "truesight",    "detect_traps",
    "scrying", "fireshield",  "shockshield",    "r1",           "iceshield",    "possess",
    "berserk", "aqua_breath", "recurringspell", "contagious",   "acidmist",     "venomshield"};

char *const smaug_lang_names[] = {
    "common", "elvish",  "dwarven", "pixie",     "ogre",    "orcish", "trollese", "rodent",  "insectoid",
    "mammal", "reptile", "dragon",  "spiritual", "magical", "goblin", "god",      "ancient", "halfling",
    "clan",   "gith",    "gnome",   "",          "",        "",       "",         "",        "",
    "",       "",        "",        "",          "",        "" /* pad to 32 for compat with smaug_flag_string */
};

char *const smaug_npc_position[] = {"dead",     "mortal",  "incapacitated", "stunned",  "sleeping",  "resting",
                                    "sitting",  "berserk", "aggressive",    "fighting", "defensive", "evasive",
                                    "standing", "mounted", "shove",         "drag"};

char *const smaug_sex[] = {"neuter", "male", "female"};

char *const smaug_npc_race[] = {/* starting from 0.... */
                                "human",      "elf",         "dwarf",      "halfling",     "pixie",      "vampire",
                                "half-ogre",  "half-orc",    "half-troll", "half-elf",     "gith",       "drow",
                                "sea-elf",    "lizardman",   "gnome",      "r5",           "r6",         "r7",
                                "r8",         "troll",       "ant",        "ape",          "baboon",     "bat",
                                "bear",       "bee",         "beetle",     "boar",         "bugbear",    "cat",
                                "dog",        "dragon",      "ferret",     "fly",          "gargoyle",   "gelatin",
                                "ghoul",      "gnoll",       "gnome",      "goblin",       "golem",      "gorgon",
                                "harpy",      "hobgoblin",   "kobold",     "lizardman",    "locust",     "lycanthrope",
                                "minotaur",   "mold",        "mule",       "neanderthal",  "ooze",       "orc",
                                "rat",        "rustmonster", "shadow",     "shapeshifter", "shrew",      "shrieker",
                                "skeleton",   "slime",       "snake",      "spider",       "stirge",     "thoul",
                                "troglodyte", "undead",      "wight",      "wolf",         "worm",       "zombie",
                                "bovine",     "canine",      "feline",     "porcine",      "mammal",     "rodent",
                                "avis",       "reptile",     "amphibian",  "fish",         "crustacean", "insect",
                                "spirit",     "magical",     "horse",      "animal",       "humanoid",   "monster",
                                "god"};

char *const smaug_npc_class[] = {"mage",    "cleric",     "thief",    "warrior", "vampire", "druid", "ranger",
                                 "augurer", "paladin",    "nephandi", "savage",  "pirate",  "pc12",  "pc13",
                                 "pc14",    "pc15",       "pc16",     "pc17",    "pc18",    "pc19",  "baker",
                                 "butcher", "blacksmith", "mayor",    "king",    "queen"};
...
char *const smaug_part_flags[] = {
    ...
    "tail",    "scales",   "claws",  "fangs",       "horns",     "tusks",     "tailattack", "sharpscales",
    "beak",    "haunches", "hooves", "paws",        "forelegs",  "feathers",  "r1",         "r2"};

char *const smaug_attack_flags[] = {
    "bite",        "claws",        "tail",         "sting",         "punch",  "kick",        "trip",
    "bash",        "stun",         "gouge",        "backstab",      "feed",   "drain",       "firebreath",
    "frostbreath", "acidbreath",   "lightnbreath", "gasbreath",     "poison", "nastypoison", "gaze",
    "blindness",   "causeserious", "earthquake",   "causecritical", "curse",  "flamestrike", "harm",
    "fireball",    "colorspray",   "weaken",       "spiralblast"};

char *const smaug_defense_flags[] = {"parry",        "dodge",       "heal",       "curelight", "cureserious",
                                     "curecritical", "dispelmagic", "dispelevil", "sanctuary", "fireshield",
                                     "shockshield",  "shield",      "bless",      "stoneskin", "teleport",
                                     "monsum1",      "monsum2",     "monsum3",    "monsum4",   "disarm",
                                     "iceshield",    "grip",        "truesight",  "acidmist",  "venomshield"};

char *const smaug_ris_flags[] = {"fire",   "cold",  "electricity", "energy", "blunt", "pierce",    "slash", "acid",
...
{
    FILE *ofp, *listfp;
    char filename[MAX_STRING_LEN], tmpstr[MAX_STRING_LEN];
    int i, j, k, x;
    int m;
    ...
    for (i = 0; i < Zones->Count; i++)
    {
        ...
        {
            snprintf(tmpstr, MAX_STRING_LEN, "#%d Dump of Zone \"%s\"[#%d]...", i + 1, remap_name(Zones->Zone[i].Name),
                     Zones->Zone[i].Number);
        ...
        fprintf(ofp, "Ranges    %d %d %d %d\n", SMAUG_AREA_LEVEL_LIMIT_LOWER,
                SMAUG_AREA_LEVEL_LIMIT_UPPER,                                /* SOFT
                                                                              * (displayed)
                                                                              * limits
                                                                              */
        ...
        for (j = 0; j < Mobs->Count; j++)
        {
                 ...
                 (Mobs->Mob[j].Number <= Zones->Zone[i].Top)))
            {
                ...
                if (Verbose)
                    fprintf(stderr, "  (%d) Writing \"%s\"[#%d]\n", j, Mobs->Mob[j].Name, Mobs->Mob[j].Number);
                else if (!Quiet)
                    spin(stderr);
                ...
                fprintf(ofp, "Stats1    %d %d %d %d %d %d\n", Mobs->Mob[j].Alignment, Mobs->Mob[j].Level,
                        Mobs->Mob[j].ToHit, Mobs->Mob[j].ArmourClass,
        ...
        for (j = 0; j < Objects->Count; j++)
        {
                 ...
                 (Objects->Object[j].Number <= Zones->Zone[i].Top)))
            {
                 ...
                 */
                val = smaug_convert_item_values(Objects->Object[j].Type, Objects->Object[j].Flags.Value[0],
                                                Objects->Object[j].Flags.Value[1], Objects->Object[j].Flags.Value[2],
                ...
                fprintf(ofp, "Stats     %d %d %d %d %d\n", Objects->Object[j].Weight, Objects->Object[j].Value,
                        Objects->Object[j].Rent, 0, /* Wiley
                                                     * doesn't
                                                     * have
                                                     * item
                                                     * levels
                ...
                for (k = 0; k < Objects->Object[j].AffectCount; k++)
                    fprintf(ofp, "Affect    %d %d %d %d %d\n", -1, -1, Objects->Object[j].Affect[k].modifier,
        ...
        for (j = 0; j < Rooms->Count; j++)
        {
                 ...
                 (Rooms->Room[j].Number <= Zones->Zone[i].Top)))
            {
                ...
                if (Verbose)
                    fprintf(stderr, "  (%d) Writing \"%s\"[#%d]\n", j, Rooms->Room[j].Name, Rooms->Room[j].Number);
                else if (!Quiet)
                    spin(stderr);
                ...
                if (Rooms->Room[j].Sector == SECT_TELEPORT && Rooms->Room[j].TeleportTime > 0 &&
        ...
        for (x = 0; x < Zones->Zone[i].Count; x++)
        {
            ...
            case ZONE_CMD_PUT:
            case ZONE_CMD_DOOR:
                fprintf(ofp, "Reset %c %d %d %d %d\n", Zones->Zone[i].Cmds[x].Command, Zones->Zone[i].Cmds[x].IfFlag,
                        Zones->Zone[i].Cmds[x].Arg[0], Zones->Zone[i].Cmds[x].Arg[1], Zones->Zone[i].Cmds[x].Arg[2]);
                break;
            case ZONE_CMD_GIVE:
            case ZONE_CMD_REMOVE:
            case ZONE_CMD_LEAD:
