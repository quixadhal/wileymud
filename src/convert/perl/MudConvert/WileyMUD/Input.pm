...
sub load_mobs {
  ...
  foreach my $vnum (sort { $a <=> $b } keys %{ $mob_data }) {
    ...
    while(<FP>) {
      ...
      push @line_set, $_;
      last if $_ =~ /^S$/;
      if( $_ =~ /^#\d+/ and $line != $mob_data->{$vnum}->{'Line'} ) {
        print STDERR "*** Invalid Mob data at line $line!\n";
        print STDERR "    Attempting to continue...\n";
        last;
      }
      $line++;
...
sub load_objects {
  ...
  foreach my $vnum (sort { $a <=> $b } keys %{ $obj_data }) {
    ...
    while(<FP>) {
      ...
      push @line_set, $_;
      last if $_ =~ /^S$/;
      if( $_ =~ /^#\d+/ and $line != $obj_data->{$vnum}->{'Line'} ) {
        print STDERR "*** Invalid Object data at line $line!\n";
        print STDERR "    Attempting to continue...\n";
        last;
      }
      $line++;
