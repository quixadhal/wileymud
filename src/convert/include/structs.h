...
typedef struct zone_cmds {
  char              Command;   /* command to execute, MOGEDRPLH:
                                * M - load mobile to room
                                * O - load object to room
                                * G - give object to previous mob
                                * E - equip object to previous mob
                                * D - set door state in room
                                * R - remove object in room
                                * P - put object in object
                                * L - load mob who is led by last M
				* H - hatred flags
                                */
...
typedef struct zone {
  ...
  int          Mode;    /* mode of zone reset:
                         * 0 - never reset
                         * 1 - only reset if no PC's are in the zone
                         * 2 - always reset on time
                         */
...
typedef struct shop {
  ...
  char        *Message[7];   /* messages from keepers (7 of them)
                              * %s is shopkeeper, %d is coin value
                              * 0 - trying to buy something not there
                              * 1 - trying to sell something not there
                              * 2 - trying to sell wrong type of item
                              * 3 - shop cannot afford to buy item
                              * 4 - player cannot afford to buy item
                              * 5 - player bought item for %d
                              * 6 - player sold item for %d
                              */
  int          MessageCount; /* redundant number of messages */
  /* int          Temper[2];  * messages when keepers lose their temper (2)
                              * 0 - player cannot afford an item:
                              *     0 - The shopkeeper spits player in the face.
                              *     1 - The shopkeeper smokes his joint.
                              * 1 - player attempts to kill shopkeeper:
                              *     0 - Shopkeeper tells player "Don't ever try
                              *         that again!", and gets the first hit.
                              *     1 - Shopkeeper tells player "I'm too
                              *         powerful for you - midget!", and
  ...
  /* int          Hours[2][2];*  Hours the shop is open...
                              * 0 - actual hours.
                              *     0 - open at hour X, use a zero in Wiley.
                              *     1 - close at hour X, Wiley 28 means never.
                              * 1 - unused.
                              */
...
/* Type C fields first */
  ...
  int         Level;           /* level, all classes in Class bitmask or warrior if none */
  dice        HitPoints;       /* hp */
