 ...
 */
        ...
        if(OldValue & ROOM_DARK) NewValue |= ISLES_ROOM_DARK;
        if(OldValue & ROOM_DEATH) NewValue |= ISLES_ROOM_IMMORTAL;
        if(OldValue & ROOM_NOMOB) NewValue |= ISLES_ROOM_NO_MOB;
        if(OldValue & ROOM_INDOORS) NewValue |= ISLES_ROOM_INDOORS;
        if(OldValue & ROOM_NOATTACK) NewValue |= ISLES_ROOM_SAFE;
        /* if(OldValue & ROOM_NOSTEAL) NewValue |= */
        if(OldValue & ROOM_NOSUMMON) NewValue |= ISLES_ROOM_NO_RECALL;
        /* if(OldValue & ROOM_NOMAGIC) NewValue |= */
        if(OldValue & ROOM_PRIVATE) NewValue |= ISLES_ROOM_PRIVATE;
