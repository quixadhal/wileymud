...
const int isles_convertsectors[] = {ISLES_SECT_INSIDE,   ISLES_SECT_INSIDE,     ISLES_SECT_CITY,
                                    ISLES_SECT_FIELD,    ISLES_SECT_FOREST,     ISLES_SECT_HILLS,
                                    ISLES_SECT_MOUNTAIN, ISLES_SECT_WATER_SWIM, ISLES_SECT_WATER_NOSWIM,
                                    ISLES_SECT_AIR,      ISLES_SECT_UNDERWATER};
...
void dump_as_isles(zones *Zones, rooms *Rooms, shops *Shops)
{
    ...
    {
        snprintf(filename, MAX_STRING_LEN, "%s/%s/%s_%d.zone", OutputDir, ISLES_SUBDIR, remap_name(Zones->Zone[i].Name),
                 Zones->Zone[i].Number);
                ...
                for (k = 0; k < Rooms->Room[j].ExitCount; k++)
                {
                    ...
                    case EXIT_ONE_WAY:
                        fprintf(ofp, "Door\t%d %d %d %d\n%s~\n", Rooms->Room[j].Exit[k].Direction, 0, /* reset
                                                                                                       ...
                                                                                                       * later...
                                                                                                       */
                                Rooms->Room[j].Exit[k].KeyNumber, Rooms->Room[j].Exit[k].Room,
