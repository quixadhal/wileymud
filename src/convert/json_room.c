...
const char *room_flag_names[64] = {
    "dark", "death", "nomob", "indoors", "noattack", "nosteal", "nosummon", "nomagic", "", "private", "sound", "", "",
    "",     "",      "",      "",        "",         "",        "",         "",        "", "",        "",      "", "",
    "",     "",      "",      "",        "",         "",        "",         "",        "", "",        "",      "", "",
    "",     "",      "",      "",        "",         "",        "",         "",        "", "",        "",      "", "",
    "",     "",      "",      "",        "",         "",        "",         "",        "", "",        "",      ""};
...
cJSON *process_room_extra_descriptions(cJSON *this_room, rooms *Rooms, int i)
{
    ...
    if (Rooms->Room[i].ExtraCount)
    {
        ...
        for (int j = 0; j < Rooms->Room[i].ExtraCount; j++)
        {
            ...
            {
                keywords = cJSON_CreateStringArray((const char *const *)Rooms->Room[i].Extra[j].Keyword->Word,
                                                   Rooms->Room[i].Extra[j].Keyword->Count);
...
cJSON *process_room_exits(cJSON *this_room, rooms *Rooms, int i, zones *Zones, objects *Objects)
{
    ...
    if (Rooms->Room[i].ExitCount)
    {
        ...
        for (int j = 0; j < Rooms->Room[i].ExitCount; j++)
        {
            ...
            {
                keywords = cJSON_CreateStringArray((const char *const *)Rooms->Room[i].Exit[j].Keyword->Word,
                                                   Rooms->Room[i].Exit[j].Keyword->Count);
    ...
    if (FoundResets > 0)
    {
        ...
        for (int j = 0; j < Zones->Count; j++)
        {
            ...
            for (int k = 0; k < Zones->Zone[j].Count; k++)
            {
                ...
                this_reset = process_reset_segment(&LastMob, &LastLoc, &LastObj, &LeaderMob, i, j, k, Rooms, Zones,
                                                   Objects, Mobs, NULL);
                if (this_reset)
                {
