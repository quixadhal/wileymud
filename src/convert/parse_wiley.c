...
const special Specials[] = {{SPECIAL_ROOM, 1014, "House", "House (Muidnar's Home)"},
...
zones *load_zones(char *infile)
{
    ...
    for (i = 0; i < Zones->Count; i++)
    {
        ...
        if (sscanf(tmp, " %d %d %d ", &(Zones->Zone[i].Top), &(Zones->Zone[i].Time), &(Zones->Zone[i].Mode)) != 3)
        ...
        for (j = Zones->Zone[i].Count = 0; (tmp = get_line(ifp, &Line, &Pos, 1));)
        {
            ...
            else if ((*tmp == ZONE_CMD_MOBILE) || (*tmp == ZONE_CMD_OBJECT) || (*tmp == ZONE_CMD_EQUIP) ||
                     (*tmp == ZONE_CMD_PUT) || (*tmp == ZONE_CMD_DOOR))
            {
                if (sscanf(tmp, "%c %d %d %d %d ", &(Zones->Zone[i].Cmds[j].Command), &(Zones->Zone[i].Cmds[j].IfFlag),
                ...
                if (Verbose)
                    fprintf(stderr, "Zone [#%d] Command %d:  %c %d %d %d %d\n", Zones->Zone[i].Number, j,
                            Zones->Zone[i].Cmds[j].Command, Zones->Zone[i].Cmds[j].IfFlag,
                            Zones->Zone[i].Cmds[j].Arg[0], Zones->Zone[i].Cmds[j].Arg[1],
                            Zones->Zone[i].Cmds[j].Arg[2]);
                else if (!Quiet)
                    spin(stderr);
            }
            else if ((*tmp == ZONE_CMD_GIVE) || (*tmp == ZONE_CMD_REMOVE) || (*tmp == ZONE_CMD_LEAD) ||
                     (*tmp == ZONE_CMD_HATE))
            {
                if (sscanf(tmp, "%c %d %d %d ", &(Zones->Zone[i].Cmds[j].Command), &(Zones->Zone[i].Cmds[j].IfFlag),
                           &(Zones->Zone[i].Cmds[j].Arg[0]), &(Zones->Zone[i].Cmds[j].Arg[1])) != 4)
                {
                    if (Debug)
                        log_error("Bad args to Zone command, %s at %d.", infile, Line);
                    continue;
            ...
            Zones->Zone[i].Cmds = (zone_cmds *)get_more_mem((char *)Zones->Zone[i].Cmds, j + 1, sizeof(zone_cmds));
                ...
                if (Rooms->Room[i].Exit[j].Room < 0)
                {
                    ...
                    if (Debug)
                        log_error("DESCRIPTION-ONLY exit found as %s exit of Room \"%s\"[#%d].",
                                  exit_name(Rooms->Room[i].Exit[j].Direction), room_name(Rooms, Rooms->Room[i].Number),
                                  Rooms->Room[i].Number);
                }
                else
                    Rooms->Room[i].Exit[j].RoomIndex = remap_room_vnum(Rooms, Rooms->Room[i].Exit[j].Room);
            }
        }
    }
    if (!Quiet)
...
void verify_exits(rooms *Rooms)
{
    ...
    for (i = 0; i < Rooms->Count; i++)
    {
        ...
        if (Rooms->Room[i].ExitCount < 1)
        {
            ...
            {
                if (Debug)
                    log_error("NO-EXITS in Room \"%s\"[#%d]!", room_name(Rooms, Rooms->Room[i].Number),
                              Rooms->Room[i].Number);
            }
            else
            {
                    ...
                    if ((real = Rooms->Room[i].Exit[j].RoomIndex) < 0)
                    {
                        ...
                        if (Debug)
                            log_error(
                                ...
                                Rooms->Room[i].Exit[j].Room, exit_name(Rooms->Room[i].Exit[j].Direction),
                                room_name(Rooms, Rooms->Room[i].Number), Rooms->Room[i].Number);
                        /*
                        ...
                        if (Rooms->Room[real].ExitCount < 1)
                        {
                            if (Debug)
                                log_error("NO-EXITS in Target Room \"%s\"[#%d]!",
                                          room_name(Rooms, Rooms->Room[real].Number), Rooms->Room[real].Number);
                        }
                        else
                        {
                            back = -1;
                            ...
                            {
                                if (Rooms->Room[real].Exit[k].Direction == RevDir[Rooms->Room[i].Exit[j].Direction])
                                    back = k;
                            }
                            if (back < 0)
                            {
                                ...
                                if (Debug)
                                    log_error("ONE-WAY %s exit in Room \"%s\"[#%d]!  No matching %s exit in Target "
                                              "Room \"%s\"[#%d].",
                            ...
                            else
                            {
                                if (Rooms->Room[real].Exit[back].Room != Rooms->Room[i].Number)
                                {
                                    ...
                                    if (Debug)
                                        log_error("NON-EUCLIDEAN mapping:  Room \"%s\"[#%d] %s -> Room \"%s\"[#%d] != "
                                                  "Room \"%s\"[#%d] %s -> Room \"%s\"[#%d]!",
                                                  room_name(Rooms, Rooms->Room[i].Number), Rooms->Room[i].Number,
                                                  exit_name(Rooms->Room[i].Exit[j].Direction),
        ...
        else if (Rooms->Room[i].Zone != CorrectZone)
        {
            if (Debug)
                log_error("Incorrect Zone ID of [#%d] for room \"%s\" [#%d],\n"
                          "Remapping to Zone [#%d].\n",
                          Rooms->Room[i].Zone, Rooms->Room[i].Name, Rooms->Room[i].Number, CorrectZone);
...
rooms *load_rooms(char *infile, zones *Zones)
{
    ...
    for (i = 0; i < Rooms->Count; i++)
    {
        ...
        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Rooms->Room[i].Name = my_strdup(tmp))))
            log_fatal("Cannot get name for Room #%d, %s at %d.", Rooms->Room[i].Number, infile, Line);

        if (Verbose)
            fprintf(stderr, "  (%d) Parsing \"%s\"[#%d]...\n", i, Rooms->Room[i].Name, Rooms->Room[i].Number);
        else if (!Quiet)
            spin(stderr);

        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Rooms->Room[i].Description = my_strdup(tmp))))
            log_fatal("Cannot get description for Room #%d, %s at %d.", Rooms->Room[i].Number, infile, Line);
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
            (sscanf(tmp, " %d %d %d ", &(Rooms->Room[i].Zone), &(Rooms->Room[i].Flags), &(Rooms->Room[i].Sector)) != 3))
        {
            if (Debug)
                log_error("Bug in MAIN flags for Room #%d, %s at %d!", Rooms->Room[i].Number, infile, Line);
        ...
        if (Rooms->Room[i].Sector == SECT_TELEPORT)
        {
            if (sscanf(tmp, " %d %d %d %d %d %d %d ", &(Rooms->Room[i].Zone), &(Rooms->Room[i].Flags),
                       &(Rooms->Room[i].Sector), &(Rooms->Room[i].TeleportTime), &(Rooms->Room[i].TeleportTo),
                       &(Rooms->Room[i].TeleportLook), &(Rooms->Room[i].TeleportSector)) != 7)
            {
                if (Debug)
                    log_error("Bad teleport flags for Room #%d, %s at %d.", Rooms->Room[i].Number, infile, Line);
        ...
        {
            if (sscanf(tmp, " %d %d %d %d %d ", &(Rooms->Room[i].Zone), &(Rooms->Room[i].Flags),
                       &(Rooms->Room[i].Sector), &(Rooms->Room[i].RiverSpeed), &(Rooms->Room[i].RiverDirection)) != 5)
            {
                Rooms->Room[i].RiverSpeed = 0;
                Rooms->Room[i].RiverDirection = -1;

                if (Debug)
                    log_error("Missing river flags for Room #%d, %s at %d.", Rooms->Room[i].Number, infile, Line);
            }
        }
        ...
        if (Rooms->Room[i].Flags & ROOM_SOUND)
        {
            ...
            for (j = 0; j < Rooms->Room[i].SoundCount; j++)
                if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Rooms->Room[i].SoundText[j] = my_strdup(tmp))))
                    log_fatal("Missing sound %d for Room #%d, %s at %d.", j, Rooms->Room[i].Number, infile, Line);
        ...
        for (j = e = Rooms->Room[i].ExitCount = Rooms->Room[i].ExtraCount = 0; (tmp = get_line(ifp, &Line, &Pos, 1));)
        {
        HolyShit:
            if (*tmp == 'S')
                break;
            else if (*tmp == 'D')
            {
                if (sscanf(tmp + 1, "%d", &(Rooms->Room[i].Exit[j].Direction)) != 1)
                    log_fatal("Corrupt Exit in Room #%d, %s at %d.", Rooms->Room[i].Number, infile, Line);
                if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) ||
                    (!(Rooms->Room[i].Exit[j].Description = my_strdup(tmp))))
                    log_fatal("No exit description for Room #%d, %s at %d.", Rooms->Room[i].Number, infile, Line);
                if (!(tmp = get_tilde_string(ifp, &Line, &Pos)))
                    log_fatal("No exit keywords for Room #%d, %s at %d.", Rooms->Room[i].Number, infile, Line);
                Rooms->Room[i].Exit[j].Keyword = make_keyword_list(tmp);
                if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                    (sscanf(tmp, " %d %d %d ", &(Rooms->Room[i].Exit[j].Type), &(Rooms->Room[i].Exit[j].KeyNumber),
                            &(Rooms->Room[i].Exit[j].Room)) != 3))
                {
                    if (Debug)
                        log_error("Bad exit type line in Room #%d, %s at %d.", Rooms->Room[i].Number, infile, Line);
                ...
                if (Rooms->Room[i].Exit[j].Error != EXIT_UNKNOWN)
                {
                    ...
                    Rooms->Room[i].Exit = (EXIT *)get_more_mem((char *)Rooms->Room[i].Exit, j + 1, sizeof(EXIT));
            ...
            {
                if (!(tmp = get_tilde_string(ifp, &Line, &Pos)))
                    log_fatal("No extra keywords for Room #%d, %s at %d.", Rooms->Room[i].Number, infile, Line);
                Rooms->Room[i].Extra[e].Keyword = make_keyword_list(tmp);
                if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) ||
                    (!(Rooms->Room[i].Extra[e].Description = my_strdup(tmp))))
                    log_fatal("No extra descriptions for Room #%d, %s of %d.", Rooms->Room[i].Number, infile, Line);
                e = ++Rooms->Room[i].ExtraCount;
                Rooms->Room[i].Extra = (extra *)get_more_mem((char *)Rooms->Room[i].Extra, e + 1, sizeof(extra));

                bzero(&(Rooms->Room[i].Extra[e]), sizeof(extra));
            }
            else
                log_fatal("Unknown line in Room #%d, %s at %d!", Rooms->Room[i].Number, infile, Line);
        }
    }
    if (Verbose)
...
shops *load_shops(char *infile)
{
    ...
    for (i = 0; i < Shops->Count; i++)
    {
        ...
        {
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Sell[j])) != 1))
                log_fatal("Cannot get item %d to sell, %s at %d.", j, infile, Line);
            if (Shops->Shop[i].Sell[j] > -1)
                Shops->Shop[i].SellCount++;
        }
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %lf ", &(Shops->Shop[i].Selling)) != 1))
            log_fatal("Cannot get Sell Profit Rate, %s at %d.", infile, Line);
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %lf ", &(Shops->Shop[i].Buying)) != 1))
            log_fatal("Cannot get Buy Profit Rate, %s at %d.", infile, Line);
        for (j = Shops->Shop[i].TradeCount = 0; j < SHOP_TRADECOUNT; j++)
        {
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Trade[j])) != 1))
        ...
        {
            if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Shops->Shop[i].Message[j] = my_strdup(tmp))))
                log_fatal("Cannot get message %d, %s at %d.", j, infile, Line);
        }
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Attitude)) != 1))
            log_fatal("Cannot get attitude (temper 0), %s at %d.", infile, Line);
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Immortal)) != 1))
            log_fatal("Cannot get immortal (temper 1), %s at %d.", infile, Line);
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Keeper)) != 1))
            log_fatal("Cannot get keeper vnum, %s at %d.", infile, Line);
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Unused)) != 1))
            log_fatal("Cannot get unused flag, %s at %d.", infile, Line);
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Room)) != 1))
            log_fatal("Cannot get room vnum, %s at %d.", infile, Line);
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Open)) != 1))
            log_fatal("Cannot get open hour, %s at %d.", infile, Line);
        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Close)) != 1))
            log_fatal("Cannot get close hour, %s at %d.", infile, Line);
        for (j = 0; j < 2; j++)
        {
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) || (sscanf(tmp, " %d ", &(Shops->Shop[i].Unused2[j])) != 1))
...
char *class_name(int Class)
{
    ...
    static char *class_name_list[] = {"mage", "cleric", "warrior", "rogue", "ranger", "druid"};
...
char *immunity_name(int Imms)
{
    ...
    static char *imm_name[] = {"Fire",  "Cold",        "Electricity", "Energy", "Blunt",  "Pierce",
                               "Slash", "Acid",        "Poison",      "Drain",  "Sleep",  "Charm",
                               "Hold",  "Non-Magical", "Plus-1",      "Plus-2", "Plus-3", "Plus-4"};
...
char *item_flag_name(int Type)
{
    ...
    static char *item_name[] = {"Glow",         "Hum",          "Metal",       "Mineral",   "Organic",
                                "Invisible",    "Magical",      "Undroppable", "Blessed",   "Anti-Good",
                                "Anti-Evil",    "Anti-Neutral", "Anti-Cleric", "Anti-Mage", "Anti-Thief",
                                "Anti-Fighter", "Anti-Ranger",  "Parishable"};
...
char *container_closeable(int Value)
{
    ...
    static char *cont_name[] = {"Closeable", "Pickproof", "Closed", "Locked"};
...
char *act_name(int Flag)
{
    ...
    static char *a_name[] = {"Special-Function", "Sentinel",  "Scavenger", "IsNPC",         "Nice-Thief",
                             "Aggressive",       "Stay-Zone", "Wimpy",     "Annoying",      "Hateful",
                             "Afraid",           "Immortal",  "Hunting",   "Deadly",        "Polymorph-Self",
                             "Polymorph-Other",  "Guardian",  "Use-Items", "Fighter-Moves", "Provide-Food",
    ...
    for (i = 0; i < Objects->Count; i++)
    {
        ...
        if (Objects->Object[i].Zone != CorrectZone)
        {
            log_error("Improper Zone ID of [#%d] for object \"%s\" [#%d],\n"
                      "Should belong to Zone [#%d].\n",
                      Objects->Object[i].Zone, Objects->Object[i].Name, Objects->Object[i].Number, CorrectZone);
            /*
             * Objects->Object[i].Zone= CorrectZone;
             */
        }
...
objects *load_objects(char *infile, zones *Zones)
{
    ...
    for (i = 0; i < Objects->Count; i++)
    {
        ...
        if (!(tmp = get_line(ifp, &Line, &Pos, 1)))
            log_fatal("Cannot get vnum for Object #%d from %s!", Objects->Object[i].Number, infile);
        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Objects->Object[i].Name = my_strdup(tmp))))
            log_fatal("Cannot get name for Object #%d, %s at %d.", Objects->Object[i].Number, infile, Line);

        if (Verbose)
            fprintf(stderr, "  (%d) Parsing \"%s\"[#%d]...\n", i, Objects->Object[i].Name, Objects->Object[i].Number);
        else if (!Quiet)
            spin(stderr);

        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Objects->Object[i].ShortDesc = my_strdup(tmp))))
            log_fatal("Cannot get short description for Object #%d, %s at %d.", Objects->Object[i].Number, infile,
                      Line);

        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Objects->Object[i].Description = my_strdup(tmp))))
            log_fatal("Cannot get description for Object #%d, %s at %d.", Objects->Object[i].Number, infile, Line);

        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Objects->Object[i].ActionDesc = my_strdup(tmp))))
            log_fatal("Cannot get action description for Object #%d, %s at %d.", Objects->Object[i].Number, infile,
                      Line);

        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
            (sscanf(tmp, " %d %d %d ", &(Objects->Object[i].Type), &(Objects->Object[i].Flags.Extra),
                    &(Objects->Object[i].Flags.Wear)) != 3))
            log_fatal("Cannot get main flags for Object #%d, %s at %d.", Objects->Object[i].Number, infile, Line);

        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
            (sscanf(tmp, " %d %d %d %d ", &(Objects->Object[i].Flags.Value[0]), &(Objects->Object[i].Flags.Value[1]),
                    &(Objects->Object[i].Flags.Value[2]), &(Objects->Object[i].Flags.Value[3])) != 4))
            log_fatal("Cannot get value flags for Object #%d, %s at %d.", Objects->Object[i].Number, infile, Line);

        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
            (sscanf(tmp, " %d %d %d ", &(Objects->Object[i].Weight), &(Objects->Object[i].Value),
                    &(Objects->Object[i].Rent)) != 3))
            log_fatal("Cannot get value flags for Object #%d, %s at %d.", Objects->Object[i].Number, infile, Line);
             ...
             (tmp = get_line(ifp, &Line, &Pos, 1));)
        {
            ...
            else if (*tmp == 'A')
            {
                if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                    ...
                    log_fatal("Cannot get affect flags for Object #%d, %s at %d.", Objects->Object[i].Number, infile,
                              Line);
                j = ++Objects->Object[i].AffectCount;
                Objects->Object[i].Affect =
                    (obj_affect *)get_more_mem((char *)Objects->Object[i].Affect, j + 1, sizeof(obj_affect));
                bzero(&(Objects->Object[i].Affect[j]), sizeof(obj_affect));
            }
            else if (*tmp == 'E')
            {
                if (!(tmp = get_tilde_string(ifp, &Line, &Pos)))
                    log_fatal("No extra keywords for Object #%d, %s at %d.", Objects->Object[i].Number, infile, Line);
                Objects->Object[i].Extra[e].Keyword = make_keyword_list(tmp);
                if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) ||
                    (!(Objects->Object[i].Extra[e].Description = my_strdup(tmp))))
                    log_fatal("No extra descriptions for Object #%d, %s of %d.", Objects->Object[i].Number, infile,
                              Line);
                e = ++Objects->Object[i].ExtraCount;
                Objects->Object[i].Extra =
                    (extra *)get_more_mem((char *)Objects->Object[i].Extra, e + 1, sizeof(extra));
                bzero(&(Objects->Object[i].Extra[e]), sizeof(extra));
            }
            else
                log_fatal("Unknown line in Object #%d, %s at %d!", Objects->Object[i].Number, infile, Line);
        }
    }
    if (Verbose)
...
char *spell_name(int Spell)
{
    ...
    if (!assigned)
    {
        ...
        ASSIGN_SPELL(SKILL_APRAISE, 0, 1, "appraise", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, 2, CLASS_ALL,
                     LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_BACKSTAB, 0, 1, "backstab", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, LOKI, 1, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_BANDAGE, 0, 1, "bandage", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, 1, CLASS_ALL,
                     LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_BARE_HAND, 0, 1, "barehand", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, 1, CLASS_ALL,
                     LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_BARTER, 0, 0, "barter", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SKILL_BLIND_FIGHTING, 0, 0, "blind fighting", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING,
                     LOKI, CLASS_ALL, LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_BREW, 0, 0, "brewing", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_DETECT_NOISE, 0, 0, "hear noise", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_DISARM, 0, 1, "disarm", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, 1, LOKI, 1, LOKI);
        ASSIGN_SPELL(SKILL_DISARM_TRAP, 0, 0, "disarm trap", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_DODGE, 0, 0, "dodge", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_DOOR_BASH, 0, 1, "doorbash", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, 1, LOKI, 1, LOKI);
        ...
        ASSIGN_SPELL(SKILL_KNOCK_OUT, 0, 0, "knockout", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_MEDITATION, 0, 1, "meditation", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, 7,
                     CLASS_ALL, LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_PARRY, 0, 0, "parry", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_PEER, 0, 1, "peer", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL, LOKI,
                     LOKI, LOKI, 1, 1, LOKI);
        ASSIGN_SPELL(SKILL_PICK_LOCK, 0, 1, "pick lock", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, LOKI, 1, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_PUNCH, 0, 1, "punch", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, 2, CLASS_ALL, LOKI,
                     LOKI, 1, LOKI, 1, LOKI);
        ASSIGN_SPELL(SKILL_READ_MAGIC, 0, 0, "read magic", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, 3,
                     CLASS_ALL, LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_RESCUE, 0, 1, "rescue", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, 1, LOKI, 1, LOKI);
        ASSIGN_SPELL(SKILL_RIDE, 0, 1, "riding", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, 1, CLASS_ALL, LOKI,
                     LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_SCRIBE, 0, 0, "scribe", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_SEARCH, 0, 1, "search", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, LOKI, 1, 1, LOKI);
        ASSIGN_SPELL(SKILL_SNEAK, 0, 1, "sneak", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, LOKI, 1, 1, LOKI);
        ...
        ASSIGN_SPELL(SKILL_SPEC_SMITE, 0, 1, "smite spec", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, 1, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_SPEC_STAB, 0, 1, "stab spec", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, 1, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_SPEC_WHIP, 0, 1, "whip spec", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, 1, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SKILL_STEAL, 0, 1, "steal", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     LOKI, LOKI, LOKI, 1, LOKI, LOKI);
        ASSIGN_SPELL(SKILL_SWIMMING, 0, 1, "swimming", NULL, 0, 100, 100, TAR_IGNORE, POSITION_STANDING, 1, CLASS_ALL,
                     LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_ARMOR, 1, 0, "armour", cast_armor, 12, 5, 50, TAR_CHAR_ROOM, POSITION_STANDING, LOKI,
                     CLASS_ALL, 4, 1, LOKI, LOKI, 10, LOKI);
        ...
        ASSIGN_SPELL(SPELL_BURNING_HANDS, 1, 0, "burning hands", cast_burning_hands, 24, 30, 50,
                     TAR_IGNORE | TAR_VIOLENT, POSITION_FIGHTING, LOKI, CLASS_ALL, 5, LOW_IMMORTAL, LOKI, LOKI, LOKI,
                     LOKI);
        ...
        ASSIGN_SPELL(SPELL_CALL_LIGHTNING, 1, 0, "call lightning", cast_call_lightning, 36, 15, 50,
                     ...
                     LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_CALM, 1, 0, "calm", cast_calm, 24, 15, 50, TAR_CHAR_ROOM, POSITION_STANDING, LOKI, CLASS_ALL,
                     4, 2, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_CAUSE_LIGHT, 1, 0, "cause light", cast_cause_light, 12, 8, 50,
                     TAR_CHAR_ROOM | TAR_FIGHT_VICT, POSITION_FIGHTING, LOKI, CLASS_ALL, LOW_IMMORTAL, 1, LOKI, LOKI,
                     LOKI, LOKI);
        ASSIGN_SPELL(SPELL_CAUSE_SERIOUS, 1, 0, "cause serious", cast_cause_serious, 12, 9, 50,
                     TAR_CHAR_ROOM | TAR_VIOLENT, POSITION_FIGHTING, LOKI, CLASS_ALL, 30, 7, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_CHARM_MONSTER, 1, 0, "charm monster", cast_charm_monster, 18, 5, 50,
                     TAR_CHAR_ROOM | TAR_VIOLENT, POSITION_STANDING, LOKI, CLASS_ALL, 8, 8, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_CHARM_PERSON, 1, 0, "charm person", cast_charm_person, 12, 5, 50,
                     TAR_CHAR_ROOM | TAR_SELF_NONO | TAR_FIGHT_VICT | TAR_VIOLENT, POSITION_STANDING, LOKI, CLASS_ALL,
                     12, 12, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_COLOUR_SPRAY, 1, 0, "colour spray", cast_colour_spray, 24, 15, 50, TAR_IGNORE | TAR_VIOLENT,
        ...
        ASSIGN_SPELL(SPELL_CONE_OF_COLD, 1, 0, "cone of cold", cast_cone_of_cold, 24, 15, 50, TAR_IGNORE | TAR_VIOLENT,
                     POSITION_FIGHTING, LOKI, CLASS_ALL, 11, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_CONJURE_ELEMENTAL, 1, 0, "conjure elemental", cast_conjure_elemental, 24, 30, 100,
                     TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL, 16, 14, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_CONT_LIGHT, 1, 0, "continual light", cast_cont_light, 24, 10, 50, TAR_IGNORE,
                     POSITION_STANDING, LOKI, CLASS_ALL, 3, 4, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_CREATE_WATER, 1, 0, "create water", cast_create_water, 12, 5, 50,
                     TAR_OBJ_INV | TAR_OBJ_EQUIP, POSITION_STANDING, LOKI, CLASS_ALL, LOW_IMMORTAL, 2, LOKI, LOKI, 10,
                     LOKI);
        ASSIGN_SPELL(SPELL_CURE_BLIND, 1, 0, "cure blindness", cast_cure_blind, 12, 5, 50, TAR_CHAR_ROOM,
        ...
        ASSIGN_SPELL(SPELL_DETECT_EVIL, 1, 0, "detect evil", cast_detect_evil, 12, 5, 50, TAR_CHAR_ROOM,
                     POSITION_STANDING, LOKI, CLASS_ALL, LOW_IMMORTAL, 1, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_DETECT_INVISIBLE, 1, 0, "detect invisible", cast_detect_invisibility, 12, 5, 50,
                     TAR_CHAR_ROOM, POSITION_STANDING, LOKI, CLASS_ALL, 2, 5, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_DETECT_POISON, 1, 0, "detect poison", cast_detect_poison, 12, 5, 50,
                     TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_EQUIP, POSITION_STANDING, LOKI, CLASS_ALL, LOW_IMMORTAL, 2,
                     LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_DISPEL_EVIL, 1, 0, "dispel evil", cast_dispel_evil, 36, 100, 100,
                     TAR_CHAR_ROOM | TAR_FIGHT_VICT, POSITION_FIGHTING, LOKI, CLASS_ALL, LOW_IMMORTAL, 12, LOKI, LOKI,
                     LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_DRAGON_BREATH, 0, 0, "DRAGON BREATH", cast_dragon_breath, 0, 100, 100,
                     TAR_IGNORE | TAR_VIOLENT, POSITION_STANDING, LOKI, CLASS_ALL, LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
#if 1
        ...
        ASSIGN_SPELL(SPELL_FAERIE_FIRE, 1, 0, "faerie fire", cast_faerie_fire, 12, 10, 50,
                     TAR_CHAR_ROOM | TAR_SELF_NONO, POSITION_STANDING, LOKI, CLASS_ALL, 5, 3, LOKI, LOKI, 11, LOKI);
        ASSIGN_SPELL(SPELL_FAERIE_FOG, 1, 0, "faerie fog", cast_faerie_fog, 24, 20, 50, TAR_IGNORE, POSITION_STANDING,
                     LOKI, CLASS_ALL, 13, 10, LOKI, LOKI, 14, LOKI);
        ...
        ASSIGN_SPELL(SPELL_FLAMESTRIKE, 1, 0, "flamestrike", cast_flamestrike, 24, 15, 50,
                     ...
                     LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_FLY, 1, 0, "fly", cast_flying, 12, 15, 50, TAR_CHAR_ROOM, POSITION_FIGHTING, LOKI, CLASS_ALL,
                     3, 14, LOKI, LOKI, 18, LOKI);
        ASSIGN_SPELL(SPELL_FLY_GROUP, 1, 0, "group fly", cast_fly_group, 18, 30, 50, TAR_IGNORE, POSITION_FIGHTING,
                     LOKI, CLASS_ALL, 8, 22, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_GOODBERRY, 1, 0, "goodberry", cast_goodberry, 12, 40, 80, TAR_IGNORE, POSITION_STANDING,
                     LOKI, CLASS_ALL, LOKI, 40, LOKI, LOKI, 25, LOKI);
        ...
        ASSIGN_SPELL(SPELL_HEAL, 1, 0, "heal", cast_heal, 18, 50, 100, TAR_CHAR_ROOM, POSITION_FIGHTING, LOKI,
        ...
        ASSIGN_SPELL(SPELL_IDENTIFY, 0, 0, "IDENTIFY", cast_identify, 1, 100, 100, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, LOKI, LOKI, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_INFRAVISION, 1, 0, "infravision", cast_infravision, 12, 7, 50, TAR_CHAR_ROOM,
                     POSITION_STANDING, LOKI, CLASS_ALL, 5, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_KNOCK, 1, 0, "knock", cast_knock, 12, 10, 50, TAR_IGNORE, POSITION_STANDING, LOKI, CLASS_ALL,
                     3, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_MON_SUM_1, 1, 0, "monsum one", cast_mon_sum1, 24, 10, 50, TAR_IGNORE, POSITION_FIGHTING,
                     LOKI, CLASS_ALL, 5, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_MON_SUM_2, 1, 0, "monsum two", cast_mon_sum2, 24, 12, 50, TAR_IGNORE, POSITION_FIGHTING,
                     LOKI, CLASS_ALL, 7, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_MON_SUM_3, 1, 0, "monsum three", cast_mon_sum3, 24, 15, 50, TAR_IGNORE, POSITION_FIGHTING,
                     LOKI, CLASS_ALL, 9, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_MON_SUM_4, 1, 0, "monsum four", cast_mon_sum4, 24, 17, 50, TAR_IGNORE, POSITION_FIGHTING,
                     LOKI, CLASS_ALL, 11, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_MON_SUM_5, 1, 0, "monsum five", cast_mon_sum5, 24, 20, 50, TAR_IGNORE, POSITION_FIGHTING,
                     LOKI, CLASS_ALL, 13, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_MON_SUM_6, 1, 0, "monsum six", cast_mon_sum6, 24, 22, 50, TAR_IGNORE, POSITION_FIGHTING,
                     LOKI, CLASS_ALL, 15, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_MON_SUM_7, 1, 0, "monsum seven", cast_mon_sum7, 24, 25, 50, TAR_IGNORE, POSITION_STANDING,
                     LOKI, CLASS_ALL, 17, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_PARALYSIS, 1, 0, "paralyze", cast_paralyze, 36, 40, 50,
                     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_VIOLENT, POSITION_FIGHTING, LOKI, CLASS_ALL, 15, 15, LOKI,
                     LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_REMOVE_PARALYSIS, 1, 0, "remove paralysis", cast_remove_paralysis, 12, 10, 50,
                     TAR_CHAR_ROOM | TAR_FIGHT_VICT, POSITION_FIGHTING, LOKI, CLASS_ALL, LOW_IMMORTAL, 4, LOKI, LOKI,
                     LOKI, LOKI);
        ASSIGN_SPELL(SPELL_REMOVE_POISON, 1, 0, "remove poison", cast_remove_poison, 12, 5, 50,
                     TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_ROOM, POSITION_STANDING, LOKI, CLASS_ALL, LOW_IMMORTAL, 5,
                     LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_SANCTUARY, 1, 0, "sanctuary", cast_sanctuary, 36, 50, 50, TAR_CHAR_ROOM, POSITION_STANDING,
                     LOKI, CLASS_ALL, LOW_IMMORTAL, 19, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_SECOND_WIND, 1, 0, "second wind", cast_second_wind, 12, 5, 50, TAR_CHAR_ROOM,
                     POSITION_STANDING, LOKI, CLASS_ALL, 12, 6, LOKI, LOKI, 16, LOKI);
        ...
        ASSIGN_SPELL(SPELL_SHOCKING_GRASP, 1, 0, "shocking grasp", cast_shocking_grasp, 12, 15, 50,
                     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_VIOLENT, POSITION_FIGHTING, LOKI, CLASS_ALL, 1, LOW_IMMORTAL,
                     LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_SLEEP, 1, 0, "sleep", cast_sleep, 24, 15, 50, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
        ...
        ASSIGN_SPELL(SPELL_STRENGTH, 1, 0, "strength", cast_strength, 12, 10, 50, TAR_CHAR_ROOM, POSITION_STANDING,
                     LOKI, CLASS_ALL, 4, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_SUCCOR, 1, 0, "succor", cast_succor, 24, 15, 50, TAR_IGNORE, POSITION_STANDING, LOKI,
                     CLASS_ALL, 21, 18, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_TELEPORT, 1, 0, "teleport", cast_teleport, 12, 33, 50, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
                     POSITION_FIGHTING, LOKI, CLASS_ALL, 8, LOW_IMMORTAL, LOKI, LOKI, LOKI, LOKI);
        ASSIGN_SPELL(SPELL_TRUE_SIGHT, 1, 0, "true sight", cast_true_seeing, 24, 20, 50, TAR_CHAR_ROOM,
                     POSITION_STANDING, LOKI, CLASS_ALL, LOW_IMMORTAL, 12, LOKI, LOKI, LOKI, LOKI);
        ...
        ASSIGN_SPELL(SPELL_WORD_OF_RECALL, 1, 0, "word of recall", cast_word_of_recall, 12, 5, 50,
                     TAR_CHAR_ROOM | TAR_SELF_ONLY, POSITION_FIGHTING, LOKI, CLASS_ALL, LOW_IMMORTAL, 10, LOKI, LOKI,
                     LOKI, LOKI);
...
mobs *load_mobs(char *infile, zones *Zones)
{
    ...
    mobs *Mobs;
    char *tmp; /* , tmp2[MAX_STRING_LEN], *tmp3; */
    char tmpa[MAX_STRING_LEN], tmpb[MAX_STRING_LEN], tmpc[MAX_STRING_LEN], tmpd[MAX_STRING_LEN], tmpe[MAX_STRING_LEN],
        tmpf[MAX_STRING_LEN];
    register int i, j;
    int ack;
    long Line, Pos;
    ...
    for (i = 0; i < Mobs->Count; i++)
    {
        ...
        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Mobs->Mob[i].Name = my_strdup(tmp))))
            log_fatal("Cannot get name for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);

        if (Verbose)
            fprintf(stderr, "  (%d) Parsing \"%s\"[#%d]...\n", i, Mobs->Mob[i].Name, Mobs->Mob[i].Number);
        else if (!Quiet)
            spin(stderr);

        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Mobs->Mob[i].ShortDesc = my_strdup(tmp))))
            log_fatal("Cannot get short description for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);

        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Mobs->Mob[i].LongDesc = my_strdup(tmp))))
            log_fatal("Cannot get long description for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);

        if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Mobs->Mob[i].Description = my_strdup(tmp))))
            log_fatal("Cannot get description for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);

        if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
            ((ack = sscanf(tmp, " %d %d %d %c %d ", &(Mobs->Mob[i].ActFlags), &(Mobs->Mob[i].AffectedBy),
                           &(Mobs->Mob[i].Alignment), &(Mobs->Mob[i].Type), &(Mobs->Mob[i].AttackCount))) < 4))
            log_fatal("Cannot get main flags for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);
        ...
        case 'S': {
            ...
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                (sscanf(tmp, " %d %d %d %s %s ", &(Mobs->Mob[i].Level), &(Mobs->Mob[i].ToHit),
                        &(Mobs->Mob[i].ArmourClass), tmpa, tmpb) != 5))
                log_fatal("Cannot get level flags for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);
            sscanf_dice(tmpa, &(Mobs->Mob[i].HitPoints.Rolls), &(Mobs->Mob[i].HitPoints.Die),
                        &(Mobs->Mob[i].HitPoints.Modifier));
            if (ack == 5)
            { /* an attack count was read in... must be > 0 */
            ...
            sscanf_dice(tmpb, &(Mobs->Mob[i].Attack[0].Rolls), &(Mobs->Mob[i].Attack[0].Die),
            ...
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                (sscanf(tmp, " %d %d %d %d ", &(Mobs->Mob[i].Gold.Modifier), &(Mobs->Mob[i].Gold.Rolls),
                        &(Mobs->Mob[i].Experience.Modifier), &(Mobs->Mob[i].Race)) < 2))
                log_fatal("Cannot get experience flags for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);
            ...
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                (sscanf(tmp, " %d %d %d %d %d %d ", &(Mobs->Mob[i].Position), &(Mobs->Mob[i].DefaultPosition),
                        &(Mobs->Mob[i].Sex), &(Mobs->Mob[i].Resistance), &(Mobs->Mob[i].Immunity),
                        &(Mobs->Mob[i].Susceptible)) < 3))
                log_fatal("Cannot get position flags for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);
            if (Mobs->Mob[i].Sex < 3)
            {
                Mobs->Mob[i].Resistance = Mobs->Mob[i].Immunity = Mobs->Mob[i].Susceptible = 0;
            }
            ...
            {
                Mobs->Mob[i].Sex = Mobs->Mob[i].Resistance = Mobs->Mob[i].Immunity = Mobs->Mob[i].Susceptible = 0;
            }
            ...
            {
                if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Mobs->Mob[i].Sound = my_strdup(tmp))))
                    log_fatal("No local sound for Mob #%d, %s of %d.", Mobs->Mob[i].Number, infile, Line);
                if ((!(tmp = get_tilde_string(ifp, &Line, &Pos))) || (!(Mobs->Mob[i].DistantSound = my_strdup(tmp))))
                    log_fatal("No local sound for Mob #%d, %s of %d.", Mobs->Mob[i].Number, infile, Line);
            }
        }
        break;
        case 'D': {
            log_fatal("Type 'D' mob #%d, %s of %d are no longer supported.", Mobs->Mob[i].Number, infile, Line);
        }
        break;
        case 'C': {
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                (sscanf(tmp, " %d %d %d %d %d %s ", &(Mobs->Mob[i].Race), &(Mobs->Mob[i].Class), &(Mobs->Mob[i].Sex),
                        &(Mobs->Mob[i].Height), &(Mobs->Mob[i].Weight), tmpa) != 6))
                log_fatal("Cannot get race flags for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);
            sscanf_dice(tmpa, &(Mobs->Mob[i].Gold.Rolls), &(Mobs->Mob[i].Gold.Die), &(Mobs->Mob[i].Gold.Modifier));
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                (sscanf(tmp, " %s %d %s %d %d %d ", tmpa, &(Mobs->Mob[i].Level), tmpb, &(Mobs->Mob[i].ArmourClass),
                        &(Mobs->Mob[i].ToHit), &(Mobs->Mob[i].AttackCount)) != 6))
                log_fatal("Cannot get experience flags for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);
            sscanf_dice(tmpa, &(Mobs->Mob[i].Experience.Rolls), &(Mobs->Mob[i].Experience.Die),
                        &(Mobs->Mob[i].Experience.Modifier));
            sscanf_dice(tmpb, &(Mobs->Mob[i].HitPoints.Rolls), &(Mobs->Mob[i].HitPoints.Die),
                        &(Mobs->Mob[i].HitPoints.Modifier));
            Mobs->Mob[i].ManaPoints = 100;
            Mobs->Mob[i].MovePoints = 100;
            /*
            ...
            for (j = 0; j < Mobs->Mob[i].AttackCount; j++)
            {
                if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                    ...
                    log_fatal("Cannot get attack line %d for Mob #%d, %s at %d.", j, Mobs->Mob[i].Number, infile, Line);
                sscanf_dice(tmpa, &(Mobs->Mob[i].Attack[j].Rolls), &(Mobs->Mob[i].Attack[j].Die),
                            &(Mobs->Mob[i].Attack[j].Modifier));
            }
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                (sscanf(tmp, " %d %d %d ", &(Mobs->Mob[i].Immunity), &(Mobs->Mob[i].Resistance),
                        &(Mobs->Mob[i].Susceptible)) != 3))
                log_fatal("Cannot get immunity flags for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                (sscanf(tmp, " %s %s %s %s %s %s ", tmpa, tmpb, tmpc, tmpd, tmpe, tmpf) != 6))
                log_fatal("Cannot get stat flags for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);
            sscanf_dice(tmpa, &(Mobs->Mob[i].Strength.Rolls), &(Mobs->Mob[i].Strength.Die),
                        &(Mobs->Mob[i].Strength.Modifier));
            sscanf_dice(tmpa, &(Mobs->Mob[i].ExtraStrength.Rolls), &(Mobs->Mob[i].ExtraStrength.Die),
                        &(Mobs->Mob[i].ExtraStrength.Modifier));
            sscanf_dice(tmpa, &(Mobs->Mob[i].Dexterity.Rolls), &(Mobs->Mob[i].Dexterity.Die),
                        &(Mobs->Mob[i].Dexterity.Modifier));
            sscanf_dice(tmpa, &(Mobs->Mob[i].Constitution.Rolls), &(Mobs->Mob[i].Constitution.Die),
                        &(Mobs->Mob[i].Constitution.Modifier));
            sscanf_dice(tmpa, &(Mobs->Mob[i].Intelligence.Rolls), &(Mobs->Mob[i].Intelligence.Die),
                        &(Mobs->Mob[i].Intelligence.Modifier));
            sscanf_dice(tmpa, &(Mobs->Mob[i].Wisdom.Rolls), &(Mobs->Mob[i].Wisdom.Die),
                        &(Mobs->Mob[i].Wisdom.Modifier));
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                (sscanf(tmp, " %d %d %d %d %d ", &(Mobs->Mob[i].SavingThrow[0]), &(Mobs->Mob[i].SavingThrow[1]),
                        &(Mobs->Mob[i].SavingThrow[2]), &(Mobs->Mob[i].SavingThrow[3]),
                        &(Mobs->Mob[i].SavingThrow[4])) != 5))
                log_fatal("Cannot get saving throws for Mob #%d, %s at %d.", Mobs->Mob[i].Number, infile, Line);
            for (j = 0; j < 5; j++)
                if (!Mobs->Mob[i].SavingThrow[j])
                    Mobs->Mob[i].SavingThrow[j] = max(20 - Mobs->Mob[i].Level, 2);
            if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                (sscanf(tmp, " %d %d %d %d ", &(Mobs->Mob[i].Position), &(Mobs->Mob[i].DefaultPosition), &ack,
            ...
            if (Mobs->Mob[i].SkillCount)
            {
                ...
                {
                    if ((!(tmp = get_line(ifp, &Line, &Pos, 1))) ||
                        (sscanf(tmp, " %d %d %d ", &(Mobs->Mob[i].Skill[j].Number), &(Mobs->Mob[i].Skill[j].Learned),
                                &(Mobs->Mob[i].Skill[j].Recognise)) != 3))
                        log_fatal("Cannot get skill line %d for Mob #%d, %s at %d.", j, Mobs->Mob[i].Number, infile,
                                  Line);
                }
            }
        }
        break;
        default:
            log_fatal("I have NO clue what kind of mob #%d, %s of %d is.", Mobs->Mob[i].Number, infile, Line);
            break;
        }
    }
    if (Verbose)
