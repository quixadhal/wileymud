...
{
    FILE *ofp, *fp2;
    char filename[MAX_STRING_LEN], domainname[MAX_STRING_LEN], dirname[MAX_STRING_LEN], subdirname[MAX_STRING_LEN];
    char mudpath[MAX_STRING_LEN], roomname[MAX_STRING_LEN], mudname[MAX_STRING_LEN], doorname[MAX_STRING_LEN],
    ...
    for (i = 0; i < Zones->Count; i++)
    {
        ...
        snprintf(dirname, MAX_STRING_LEN, "%s/%s%s/%s", OutputDir, NIGHTMARE_SUBDIR, NIGHTMARE_DOMAIN, domainname);
                ...
                if (Rooms->Room[j].ExitCount > 0)
                {
                    ...
                    for (k = 0; k < Rooms->Room[j].ExitCount; k++)
                    {
                        ...
                        case EXIT_ONE_WAY:
                            ...
                            snprintf(
                                outpath, MAX_STRING_LEN, "%s/%s_%d/room", NIGHTMARE_DOMAIN,
                                remap_name(
                                    Zones
                                        ->Zone[remap_zone_vnum(Zones, Rooms
                                                                          ->Room[remap_room_vnum
                    ...
                    for (k = 0; k < Rooms->Room[j].ExitCount; k++)
                    {
                        ...
                        default:
                            snprintf(doorname, MAX_STRING_LEN, "%s/etc/%s_%d_%s_door.c", dirname,
                                     remap_name(room_name(Rooms, Rooms->Room[j].Exit[k].Room)),
                                     Rooms->Room[j].Exit[k].Room, exit_name_lower(Rooms->Room[j].Exit[k].Direction));
                            fprintf(ofp, "    SetDoor( \"%s\", \"%s\" );\n",
                                    exit_name_lower(Rooms->Room[j].Exit[k].Direction), doorname);
