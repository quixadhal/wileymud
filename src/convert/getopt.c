...
{
    /*
     * XXX This is no good solution.  We should rather copy the args so that we can compare them later.  But we must
     * not use malloc(3).  
     */
    original_argc = argc;
    original_argv = argv;
    ...
    char                                  **argv;
{
     ...
     * right place. It leaves the longer segment in the right place overall, but it consists of two parts that need to
     * be swapped next.  
     */

#ifdef _LIBC
    /*
     * First make sure the handling of the `__getopt_nonoption_flags' string can work normally.  Our top argument must
     * be in the range of the string.  
     */
...
{
    /*
     * Start processing options with ARGV-element 1 (since ARGV-element 0 is the program name); the sequence of
    ...
    int long_only;
{
    ...
    /*
     * Test whether ARGV[optind] points to a non-option argument. Either it does not have option syntax, or there is an 
     * environment flag from the shell indicating it is not an option.  The later information is only used when the
     * used in the GNU libc.  
     */
#ifdef _LIBC
...
#define NONOPTION_P (argv[optind][0] != '-' || argv[optind][1] == '\0')
	...
	if (ordering == PERMUTE) {
	    /*
	     * If we have just processed some options following some non-options, exchange them so that the options
	     * come first.  
	     */
	...
	/*
	 * If we have come to a non-option and did not permute it, either stop the scan or describe it to the caller
	 * and pass it by.  
	 */
     ...
     * 
     * If long_only and the ARGV-element has the form "-f", where f is a valid short option, don't consider it an
     * abbreviated form of a long option that starts with f.  Otherwise there would be no way to give the -f short
     * option.
     * 
	...
	/*
	 * Can't find it as a long option.  If this is not getopt_long_only, or the option starts with '--' or is not a 
	 * valid short option, then it's an error. Otherwise interpret it as a short option.  
	 */
...
%s: option `-W %s' doesn't allow an argument\n"), argv[0], pfound->name);
	...
	if (temp[1] == ':') {
		...
		if (*nextchar != '\0') {
		    optarg = nextchar;
		    /*
		     * If we end this ARGV-element by taking the rest as an arg, we must advance to the next element
		     * now.  
		     */
		    optind++;
		} else if (optind == argc) {
