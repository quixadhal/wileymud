...
char *WelcomeMsg = "Welcome to the WileyMUD III conversion project!\n"
                   "\n"
...
#define sub_dir(x)                                                                                                     \
    {                                                                                                                  \
        if (!strcmp("", x))                                                                                            \
            snprintf(muddir, MAX_STRING_LEN, "%s%s%s%s", FR_DOMAIN, OneBigDomain ? MEGA_DOMAIN : "",                   \
                     OneBigDomain ? "" : "/", OneBigDomain ? "" : domainname);                                         \
        else                                                                                                           \
            snprintf(muddir, MAX_STRING_LEN, "%s%s%s%s%s%s", FR_DOMAIN, OneBigDomain ? MEGA_DOMAIN : "",               \
                     OneBigDomain ? "/" x : "", *domainname ? "/" : "", domainname, OneBigDomain ? "" : "/" x);        \
        snprintf(filename, MAX_STRING_LEN, "mkdir -p %s/%s%s", OutputDir, FR_SUBDIR, muddir);                          \
        system(filename);                                                                                              \
    }
...
void setup_dirs(zones *Zones, rooms *Rooms)
{
    ...
    if (OneBigDomain)
    {
        if (!Quiet)
            spin(stderr);
        sub_dir("");
        snprintf(filename, MAX_STRING_LEN, "%s/%s%s/README", OutputDir, FR_SUBDIR, muddir);
    ...
    for (i = 0; i < Zones->Count; i++)
    {
        ...
        fprintf(ofp, ZoneDataMsg, Zones->Zone[i].Name, LowRoom, HighRoom, Zones->Zone[i].Time, Zones->Zone[i].Mode);
        fclose(ofp);
    }
    if (!Quiet)
        fprintf(stderr, "done.\n");
        ...
        for (j = 0; j < Rooms->Count; j++)
        {
                 ...
                 (Rooms->Room[j].Number <= Zones->Zone[i].Top)))
            {
                ...
                */
                ...
                snprintf(filename, MAX_STRING_LEN, "%s/%s%s/%s", OutputDir, FR_SUBDIR, muddir, roomname);
                ofp = open_file(filename, "w");
                ...
                for (k = 0; k < Rooms->Room[j].ExitCount; k++)
                {
                    ...
                    case EXIT_ONE_WAY:
                        ...
                        snprintf(
                            outpath, MAX_STRING_LEN, "%s%s%s/%s_%d%s", FR_DOMAIN, OneBigDomain ? MEGA_DOMAIN : "",
                            OneBigDomain ? "/rooms" : "",
                            remap_name(
                                Zones
