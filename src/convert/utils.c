...
char *get_line(FILE *fp, long *Line, long *Pos, int Skip) {
  ...
  do {
    ...
    *Pos= ftell(fp);
    if(!fgets(tmp, MAX_LINE_LEN-1, fp)) return NULL;
    (*Line)++;
    if(*tmp) if(tmp[strlen(tmp)-1] == '\n') tmp[strlen(tmp)-1]= '\0';
    if(Skip) {
      if(!*tmp) continue;       /* ignore blank lines */
      if(*tmp == '*') continue; /* lines starting with '*' are comments */
    }
    break;
  } while(!feof(fp));
  return tmp;
...
char *remap_name(char *Old)
{
  ...
  for(j= i= 0; i< strlen(Old); i++) {
    if(isalpha(Old[i])) Block[j++]= Old[i];
    else if((Old[i]== '_') || (Old[i]== ' ') || (Old[i]== '/')
                           || (Old[i]== '\\') || (Old[i]== '-')) continue;
    else if((Old[i]== '\'') || (Old[i]== '"')) continue;
    else break;
  }
  if(*Block) {
...
keyword *make_keyword_list(char *String) {
  ...
  if(!String || !*String) return Keyword;
