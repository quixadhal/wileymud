...
bool I3_read_config( int mudport )
{
   ...
   for( ;; )
   {
      ...
      if( !strcasecmp( word, "I3CONFIG" ) && this_i3mud == NULL )
      {
         ...
         this_i3mud->minlevel = 1; /* Minimum default level before I3 will acknowledge you exist */
...
void i3_loop( void )
{
   ...
   static struct timeval last_time, null_time;
   bool rfound = FALSE;
...
I3_CMD( I3_mudlist )
{
   ...
   for( mud = first_mud; mud; mud = mud->next )
   {
      ...
      if( filter[0] && i3_str_prefix( filter, mud->name ) &&
          ( mud->mud_type && i3_str_prefix( filter, mud->mud_type ) ) && ( mud->mudlib && i3_str_prefix( filter, mud->mudlib ) ) )
         continue;
