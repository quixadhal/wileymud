...
struct index_data *generate_indices(FILE *fl, int *top)
{
  ...
  char buf[82];
if(DEBUG) dlog("generate_indeces");  
  rewind(fl);
    ...
    if (fgets(buf, sizeof(buf), fl))  
    {
      if (*buf == '#')        
      {
        if (!i)            /* first cell */
          ...
          if (!(index = (struct index_data*) realloc(index, (i + 1) * sizeof(struct index_data)))) 
          {
            perror("load indices");
            exit(0);
          }
...
void completely_cleanout_room(struct room_data *rp)
{
  struct char_data  *ch;
  struct obj_data *obj;
if(DEBUG) dlog("completely_cleanout_room");  
  while (rp->people) {
    ch = rp->people;
...
void setup_dir(FILE *fl, int room, int dir)
{
  ...
  switch(tmp)
  {
    case 1: rp->dir_option[dir]->exit_info = EX_ISDOOR;break;
    case 2: rp->dir_option[dir]->exit_info = EX_ISDOOR|EX_PICKPROOF;break;
    case 3: rp->dir_option[dir]->exit_info = EX_ISDOOR|EX_SECRET;break;
    case 4: rp->dir_option[dir]->exit_info = EX_ISDOOR|EX_SECRET|EX_PICKPROOF;break;
...
void renum_zone_table(void)
{
  ...
  char  buf[256];
  if(DEBUG) dlog("renum_zone_table");
...
void reset_zone(int zone)
{
  ...
  char buf[256];
  struct char_data *mob,*mob2;
  struct obj_data *obj, *obj_to;
  struct room_data  *rp;
  struct char_data *last_mob_loaded;

if(DEBUG) dlog("reset_zone");
...
int create_entry(char *name)
{
  ...
  if (top_of_p_table == -1 || player_table == NULL ) {
    ...
    if (!(player_table = (struct player_index_element *) realloc(player_table, sizeof(struct player_index_element) * (++top_of_p_table + 1))))
    {
      perror("create entry");
      exit(1);
    }
...
void reset_char(struct char_data *ch)
{
  ...
  if (!GET_RACE(ch)) GET_RACE(ch) = RACE_HUMAN;
