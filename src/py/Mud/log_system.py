...
class Logger(object):
    ...
    class LogReformatter(logging.Formatter):
        ...
        As a final tweak, we use UTC as the default timezone, since that doesn't
        have issues with DST conversions and other nonsense.

        :return: An object which does proper formatting of our log entries
        :rtype: object
        """

        time_converter = datetime.datetime.utcfromtimestamp
    ...
    def __new__(cls):
        if cls.__instance is None:
            ...
            logging.Logger.start_filelog = cls.start_filelog
            logging.Logger.stop_filelog = cls.stop_filelog
    ...
    @classmethod
    def start_filelog(cls, pathname):
        if pathname is not None:
            f = cls.__formatter
            h = logging.FileHandler(pathname)
            h.setFormatter(f)
            cls.__instance.addHandler(h)
            cls.__filehandler = h
    ...
    @classmethod
    def stop_filelog(cls):
        if cls.__filehandler is not None:
            cls.__filehandler.close()
            cls.__instance.removeHandler(cls.__filehandler)
            cls.__filehandler = None
