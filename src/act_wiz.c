                ...
                for (i = 0; i < MAX_OBJ_AFFECT; i++)
                {
                    if (j->affected[i].location == APPLY_DAMROLL || j->affected[i].location == APPLY_HITNDAM)
                    {
                        ...
                        case 3:
                            ...
                            break;
                        case 4:
                            strlcat(buf, "This is definitately blessed by the gods.\r\n", MAX_STRING_LENGTH);
                            break;
                        case 5:
                            strlcat(buf, "It is ready to lose its hilt.\r\n", MAX_STRING_LENGTH);
                            break;
                        default:
                            strlcat(buf, "It is checked badly and most likely will break.\r\n", MAX_STRING_LENGTH);
                            break;
                        }
                    }
                }
...
void do_stat(struct char_data *ch, const char *argument, int cmd)
{
    ...
    if (!str_cmp("room", type) || !str_cmp("here", type))
    {
        ...
        cprintf(ch, "%s [#%d], in Zone %s [#%d] is %s.\r\n", rm->name, rm->number, zone_table[rm->zone].name, rm->zone,
                buf2);
        if (rm->tele_targ > 0)
        { /* teleport room */
            ...
            cprintf(ch, "Teleports to %s [#%d] every %3.1lf second%s", rp ? rp->name : "Swirling CHAOS", rm->tele_targ,
                    ttime, (ttime != 1.0) ? "s.\r\n" : ".\r\n");
        }
        if ((rm->sector_type == SECT_WATER_SWIM) || (rm->sector_type == SECT_WATER_NOSWIM))
        {
            if (rm->river_dir != -1 && rm->dir_option[rm->river_dir])
            {
                ...
                cprintf(ch, "A River flows %s into %s [#%d] every %3.1lf second%s", dirs[rm->river_dir],
                        rp ? rp->name : "Swirling CHAOS", rp ? rp->number : -1, ttime,
                        (ttime != 1.0) ? "s.\r\n" : ".\r\n");
            }
        }
        if (rm->room_flags)
        ...
        for (i = 0; i < MAX_NUM_EXITS; i++)
        {
            if (rm->dir_option[i])
            {
                ...
                cprintf(ch, "Exit %s to %s [#%d] is called %s.\r\n", dirs[i], rp ? rp->name : "Swirling CHAOS",
                        rp ? rp->number : -1, rm->dir_option[i]->keyword ? rm->dir_option[i]->keyword : dirs[i]);
    ...
    else if (!str_cmp("mob", type) || !str_cmp("pc", type))
    {
        ...
        snprintf(buf2, MAX_STRING_LENGTH, " :  Level [%d/%d/%d/%d/%d] : Alignment[%d]\r\n", k->player.level[0],
                 k->player.level[1], k->player.level[2], k->player.level[3], k->player.level[4], GET_ALIGNMENT(k));
        ...
        cprintf(ch, "Mana p.:[%d/%d+%d]  Hit p.:[%d/%d+%d]  Move p.:[%d/%d+%d]\r\n", GET_MANA(k), mana_limit(k),
                mana_gain(k), GET_HIT(k), hit_limit(k), hit_gain(k), GET_MOVE(k), move_limit(k), move_gain(k));

        cprintf(ch, "AC:[%d/10], Coins: [%d], Exp: [%d], Hitroll: [%d], Damroll: [%d]\r\n", GET_AC(k), GET_GOLD(k),
                GET_EXP(k), k->points.hitroll, k->points.damroll);
        ...
        if (IS_MOB(k))
        {
            cprintf(ch, "\r\nMobile Special procedure : %s\r\n",
                    (mob_index[k->nr].func ? MobFunctionNameByFunc(mob_index[k->nr].func) : "None"));
        }
    ...
    else if (!str_cmp("obj", type))
    {
        ...
        case ITEM_KEY:
            ...
            break;
        case ITEM_FOOD:
            snprintf(buf, MAX_STRING_LENGTH, "Makes full : %d\r\nPoisoned : %d", j->obj_flags.value[0],
                     j->obj_flags.value[3]);
            break;
        default:
            snprintf(buf, MAX_STRING_LENGTH, "Values 0-3 : [%d] [%d] [%d] [%d]", j->obj_flags.value[0],
                     j->obj_flags.value[1], j->obj_flags.value[2], j->obj_flags.value[3]);
            break;
        }
        cprintf(ch, "%s", buf);
...
void do_set(struct char_data *ch, const char *argument, int cmd)
{
    ...
    const char *pset_list[] = {"align", "exp",  "sex",  "race", "tohit",  "dmg",  "bank", "gold", "prac",   "str",
                               "int",   "wis",  "dex",  "con",  "stradd", "hit",  "mhit", "mana", "mmana",  "move",
                               "mmove", "mlvl", "clvl", "wlvl", "tlvl",   "rlvl", "dlvl", "aggr", "wander", NULL};
    ...
    if (victim == ch)
    {
        ...
        if (ch->desc->snoop.snooping)
        {
            if (ch->desc->snoop.snooping->desc)
                ...
                log_info("caught %s snooping %s who didn't have a descriptor!", ch->player.name,
...
void do_return(struct char_data *ch, const char *argument, int cmd)
{
    ...
    else
    {
        ...
        if ((IS_SET(ch->specials.act, ACT_POLYSELF) || IS_SET(ch->specials.act, ACT_POLYOTHER)) && cmd)
        {
            mob = ch;
...
void do_show(struct char_data *ch, const char *argument, int cmd)
{
    ...
    if (is_abbrev(buf, "zones"))
    {
        ...
        for (zone = 0; zone <= top_of_zone_table; zone++)
        {
            ...
            snprintf(buf, MAX_STRING_LENGTH, "%4d %-40s %4dm %4dm %6d-%-6d %s\r\n", zone, zd->name, zd->lifespan,
                     zd->age, zone_bottom, zd->top, mode);
            append_to_string_block(&sb, buf);
