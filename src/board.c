...
if(DEBUG) dlog("board_remove_msg");  
...
{
  int ind, len;
if(DEBUG) dlog("board_save_board");  
  if (!b) return;
...
{
  int ind, len = 0;
if(DEBUG) dlog("board_load_board");  
  OpenBoardFile(b);
  board_reset_board(b);
...
{
  int ind;
if(DEBUG) dlog("board_reset_board");  
...
int board_display_msg(struct char_data *ch, char *arg, struct Board *b) 
{
  ...
  int msg;
if(DEBUG) dlog("board_display_msg");  
  one_argument(arg, number);
  if (!*number || !isdigit(*number))
    return(0);
  if (!(msg = atoi(number))) return(0);
...
void board_fix_long_desc(struct Board *b) 
{
if(DEBUG) dlog("board_fix_long_desc");  
  return;
}
...
int board_show_board(struct char_data *ch, char *arg, struct Board *b)
{
 ...
 if(DEBUG) dlog("board_show_board"); 
