...
int 
split_string(char *str, char *sep, char **argv)
     /*
...
void 
init_string_block(struct string_block *sb)
{
  if (DEBUG)
...
void 
append_to_string_block(struct string_block *sb, char *str)
{

  int                              len;

  if (DEBUG)
    dlog("append_to_string");
...
void 
page_string_block(struct string_block *sb, struct char_data *ch)
{
  if (DEBUG)
    dlog("page_string_block");
  page_string(ch->desc, sb->data, 1);
}

void 
destroy_string_block(struct string_block *sb)
{
  if (DEBUG)
...
void 
affect_modify(struct char_data *ch, byte loc, byte mod, long bitv, bool add)
{
  int                              maxabil;
  int                              i;
 ...
 */
void 
affect_from_char(struct char_data *ch, short skill)
{
  struct affected_type            *hjp;

  if (DEBUG)
    dlog("affect_from_char");
 ...
 */
bool 
affected_by_spell(struct char_data *ch, short skill)
{
  struct affected_type            *hjp;

  if (DEBUG)
    dlog("affected_by_spell");
  for (hjp = ch->affected; hjp; hjp = hjp->next)
    if (hjp->type == skill)
      return (TRUE);
...
obj_from_char(struct obj_data *object)
{
  ...
  object->equipped_by = 0;	/*
				 * should be unnecessary, but, why risk it 
				 */
  object->next_content = 0;
}
 ...
 */
struct char_data                *
get_char_vis_world(struct char_data *ch, char *name, int *count)
{
  struct char_data                *i;
  int                              j,
                                   number;
...
struct obj_data                 *
get_obj_vis_world(struct char_data *ch, char *name,
		  int *count)
{
  struct obj_data                 *i;
  int                              j,
...
struct obj_data                 *
get_obj_vis_accessible(struct char_data *ch, char *name)
{
  struct obj_data                 *i;
  int                              j,
                                   number;
