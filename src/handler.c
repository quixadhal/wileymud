...
{
  if(DEBUG) dlog("init_string_block");
...
void append_to_string_block(struct string_block *sb, char *str)
{
  ...
  int	len;
  if(DEBUG) dlog("append_to_string");
...
{
  struct affected_type *hjp;
if(DEBUG) dlog("affect_from_char");  
...
{
  struct affected_type *hjp;
if(DEBUG) dlog("affected_by_spell");  
  for (hjp = ch->affected; hjp; hjp = hjp->next)
    if ( hjp->type == skill )
      return( TRUE );
...
{
  struct obj_data *i;
if(DEBUG) dlog("get_obj_in_list_num");  
...
{
if(DEBUG) dlog("object_list_new_owner");
  if (list) {
...
void update_char_objects( struct char_data *ch )
{
  ...
  if(DEBUG) dlog("update_char_objects");
  if (ch->equipment[WEAR_LIGHT])
    if (ch->equipment[WEAR_LIGHT]->obj_flags.type_flag == ITEM_LIGHT)
...
struct obj_data *get_obj_in_list_vis(struct char_data *ch, char *name, struct obj_data *list)
{
  ...
  if(DEBUG) dlog("get_obj_in_list_vis");
  strcpy(tmpname,name);
  tmp = tmpname;
  if(!(number = get_number(&tmp)))
    return(0);
				   ...
				   int *count)
{
  ...
  if(DEBUG) dlog("get_obj_vis_world");
  strcpy(tmpname,name);
  tmp = tmpname;
  if(!(number = get_number(&tmp)))
    return(0);
...
struct obj_data *get_obj_vis_accessible(struct char_data *ch, char *name)
{
  ...
  if(DEBUG) dlog("get_obj_vis_accessible");
  strcpy(tmpname,name);
  tmp = tmpname;
  if(!(number = get_number(&tmp)))
    return(0);
