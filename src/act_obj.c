...
*/
        if (CheckForAnyTrap(ch, obj_object)) return;
    ...
    case 2:
    {
      ...
      if ((p = getabunch(arg1,newarg))!= FALSE) 
      {
        strcpy(arg1,newarg);
        num = p;
      }
        ...
        if (GET_ITEM_TYPE(sub_object)==ITEM_CONTAINER)
	{
	  ...
	  {
            if (CheckForGetTrap(ch, obj_object)) return;
    ...
    case 6:{
      ...
      if (sub_object) {
        if (GET_ITEM_TYPE(sub_object) == ITEM_CONTAINER) {
          ...
          if (obj_object) 
	  {
            if (CheckForInsideTrap(ch, sub_object)) return;
            if ((IS_CARRYING_N(ch) + 1 < CAN_CARRY_N(ch))) {
              if ((IS_CARRYING_W(ch) + obj_object->obj_flags.weight) < 
                CAN_CARRY_W(ch)) {
                if (CAN_WEAR(obj_object,ITEM_TAKE)) {
...
if(DEBUG) dlog("weight_change_object");
...
if(DEBUG) dlog("name_from_drinkcon");
...
if(DEBUG) dlog("name_to_drinkcon");
