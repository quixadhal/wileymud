...
void argument_split_2(const char *argument, char *first_arg, char *second_arg)
{
    ...
    if (DEBUG > 2)
        log_info("called %s with %s, %s, %s", __PRETTY_FUNCTION__, VNULL(argument), VNULL(first_arg),
                 VNULL(second_arg));
...
{
    if (DEBUG > 2)
        log_info("called %s with %s, %s, %08zx, %08zx", __PRETTY_FUNCTION__, SAFE_NAME(ch), VNULL(arg),
                 (size_t)equipment, (size_t)j);
...
void list_obj_to_char(struct obj_data *list, struct char_data *ch, int mode, char show)
{
    ...
    if (DEBUG > 2)
        log_info("called %s with %08zx, %s, %d, '%c'", __PRETTY_FUNCTION__, (size_t)list, SAFE_NAME(ch), mode, show);
    ...
    else if (mode == 1)
    {
        ...
        if (IS_PC(i))
        {
            if (ch == i)
            {
                ...
                {
                    snprintf(buffer, MAX_STRING_LENGTH, "You are %s and bear the Mark of Dread Quixadhal.\r\n",
                             RaceName[GET_RACE(i)]);
                }
                else
                {
                    snprintf(buffer, MAX_STRING_LENGTH, "You are %s.\r\n", RaceName[GET_RACE(i)]);
                }
            }
            else
            {
                if (IS_SET(ch->specials.new_act, NEW_PLR_KILLOK))
                {
                    if (IS_SET(i->specials.new_act, NEW_PLR_KILLOK))
                    {
                        snprintf(buffer, MAX_STRING_LENGTH, "%s is %s and also bears the Mark of the Dread Lord.\r\n",
        ...
        if (MOUNTED(i))
            act("$n is mounted on %s", FALSE, i, 0, ch, TO_VICT, MOUNTED(i)->player.short_descr);
        if (RIDDEN(i))
...
void list_char_in_room(struct char_data *list, struct char_data *ch)
{
    ...
    {
        if ((ch != i) && (!RIDDEN(i) || (RIDDEN(i) && !CAN_SEE(ch, RIDDEN(i)))) &&
            (IS_AFFECTED(ch, AFF_SENSE_LIFE) || (CAN_SEE(ch, i) && !IS_AFFECTED(i, AFF_HIDE))))
        {
            if ((cond_top < 50) && !MOUNTED(i))
            {
                ...
                if (IS_NPC(i))
                {
                    for (k = 0; (k < cond_top && !found); k++)
                    {
                        if (cond_top > 0)
                        {
                            if (i->nr == cond_ptr[k]->nr && (GET_POS(i) == GET_POS(cond_ptr[k])) &&
...
void list_char_to_char(struct char_data *list, struct char_data *ch, int mode)
{
    ...
    if (DEBUG > 2)
        log_info("called %s with %08zx, %s, %d", __PRETTY_FUNCTION__, (size_t)list, SAFE_NAME(ch), mode);
...
void _normal_look(struct char_data *ch, const char *argument, int cmd)
{
    ...
    static const char *keywords[] = {"north", "east", "south", "west", "up",
    ...
    else if ((IS_DARKOUT(ch->in_room)) && (!IS_IMMORTAL(ch)) && (!IS_AFFECTED(ch, AFF_TRUE_SIGHT)))
    {
    ...
    else
    {
        ...
        case 5: {
            ...
            if (exitp != NULL)
            {
                ...
                {
                    if ((strcmp(fname(exitp->keyword), "secret")) && (IS_NOT_SET(exitp->exit_info, EX_SECRET)))
                    {
            ...
            if ((exitp != NULL) && (IS_AFFECTED(ch, AFF_SCRYING) || IS_IMMORTAL(ch) || cmd == SKILL_PEER))
            {
                struct room_data *rp;
                ...
                if (exitp != NULL)
                {
                    if (IS_SET(exitp->exit_info, EX_CLOSED) && (exitp->keyword) && cmd == SKILL_PEER)
                    {
                        if ((strcmp(fname(exitp->keyword), "secret")) && (IS_NOT_SET(exitp->exit_info, EX_SECRET)))
                        {
                            cprintf(ch, "The %s is closed.\r\n", fname(exitp->keyword));
        ...
        case 7: {
            if (*arg2)
            {
                bits = generic_find(arg2, FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP | FIND_CHAR_ROOM, ch, &tmp_char,
                                    &found_object);
                if (tmp_char)
                {
...
int list_exits_in_room(struct char_data *ch)
{
    ...
    for (door = 0; door < MAX_NUM_EXITS; door++)
    {
        ...
        {
            scprintf(buf, MAX_INPUT_LENGTH, "%s [#%d]%s, ", exits[door], exitdata->to_room,
                ...
                if (IS_IMMORTAL(ch))
                {
                    ...
                    if (target_room)
                    {
                        ...
                        scprintf(buf, MAX_INPUT_LENGTH, "  %s%s - [#%d]",
                                 (target_zone == this_zone) ? "\x1b[37;1m" : "\x1b[33;1m", exits[door],
                                 this_exit->to_room);
                        ...
                        scprintf(buf, MAX_INPUT_LENGTH, " %s%s",
                                 (target_exit && (target_exit->to_room == this_room->number)) ? "\x1b[32;1m"
                                                                                              : "\x1b[31;1m",
                        ...
                        if (target_exit && target_exit->to_room != this_room->number)
                        {
                            ...
                            scprintf(buf, MAX_INPUT_LENGTH, "\x1b[31;1m returns to [#%d]", target_exit->to_room);
                        }

                        if (target_zone != this_zone)
                        {
                            scprintf(buf, MAX_INPUT_LENGTH, "\x1b[33;1m in #%d - %s", target_room->zone,
                                     zone_table[target_room->zone].name);
                        }
                    ...
                    {
                        scprintf(buf, MAX_INPUT_LENGTH, "  \x1b[31;1m%s - [#%d] Swirling CHAOS!!!\x1b[0m", exits[door],
                                 this_exit->to_room);
                    }
                    strlcat(buf, "\r\n", MAX_INPUT_LENGTH);
                }
                else
                {
                    ...
                    if (target_room)
                    {
                        if (IS_SET(this_exit->exit_info, EX_CLOSED))
                            continue;
                        scprintf(buf, MAX_INPUT_LENGTH, "  %s - %s\r\n", exits[door],
                                 (IS_DARK(this_exit->to_room) || IS_DARKOUT(this_exit->to_room)) ? "Too dark to tell"
                                                                                                 : target_room->name);
                    }
                }
            }
...
int do_score(struct char_data *ch, const char *argument, int cmd)
{
    ...
    cprintf(ch, "%s%s%s %s %s is %d years old.\r\n", (IS_SET(target->specials.new_act, NEW_PLR_KILLOK)) ? "Black " : "",
            ...
            GET_PRETITLE(target) ? GET_PRETITLE(target) : "", GET_NAME(target), GET_TITLE(target), GET_AGE(target));
    ...
    if (GetMaxLevel(ch) > 3)
    {
        ...
        if (GET_ADD(target) > 99)
            cprintf(ch, "      | STR: %2d/**    DEX: %2d    CON: %2d    INT: %2d    WIS: %2d  |\r\n", GET_STR(target),
                    GET_DEX(target), GET_CON(target), GET_INT(target), GET_WIS(target));
        else
            cprintf(ch, "      | STR: %2d/%2d    DEX: %2d    CON: %2d    INT: %2d    WIS: %2d  |\r\n", GET_STR(target),
                    GET_ADD(target), GET_DEX(target), GET_CON(target), GET_INT(target), GET_WIS(target));
    ...
    if (MOUNTED(target))
    {
        if ((GET_LEVEL(target, RANGER_LEVEL_IND) > 0) || IS_IMMORTAL(ch) || IS_AFFECTED(MOUNTED(target), AFF_CHARM))
        {
            cprintf(ch, "%s has %d/%d Hit Points, and %d/%d Movement.\r\n", GET_SDESC(MOUNTED(target)),
                    GET_HIT(MOUNTED(target)), GET_MAX_HIT(MOUNTED(target)), GET_MOVE(MOUNTED(target)),
                    GET_MAX_MOVE(MOUNTED(target)));
        }
    }
    ...
    case POSITION_FIGHTING:
        if (target->specials.fighting)
            act("You are trying to KILL $N.\r\n", FALSE, ch, 0, target->specials.fighting, TO_CHAR);
        else
    ...
    playing_time = real_time_passed((time(0) - target->player.time.logon) + target->player.time.played, 0);
    cprintf(ch, "You have scored %d Experience Points in %d days and %d hours!\r\n", GET_EXP(target), playing_time.day,
            playing_time.hours);
    ...
    for (j = 0; j < ABS_MAX_CLASS; j++)
    {
        if (GET_LEVEL(target, j) > 0)
        {
            ...
            cprintf(ch, "As a %d%s level %s, you need %d xp and %d gold to become ", GET_LEVEL(target, j),
                    ordinal(GET_LEVEL(target, j)), class_name[j], EXP_NEEDED(target, j), GOLD_NEEDED(target, j));
...
int do_weather(struct char_data *ch, const char *argument, int cmd)
{
    ...
    static const char *wind_dir[] = {"North", "East", "South", "West", "Still", "None"};
...
static void where_object(struct char_data *ch, struct obj_data *obj, int recurse, struct string_block *sb)
{
    ...
    if (DEBUG > 2)
        log_info("called %s with %s, %s, %d, %08zx", __PRETTY_FUNCTION__, SAFE_NAME(ch), SAFE_ONAME(obj), recurse,
                 (size_t)sb);

    if (obj->in_room != NOWHERE)
    { /* object in a room */
        snprintf(buf, MAX_STRING_LENGTH, "%-30s- %s [%d]\r\n", obj->short_description, real_roomp(obj->in_room)->name,
                 obj->in_room);
    }
    ...
    for (i = character_list; i; i = i->next)
        if (isname(name, i->player.name) && CAN_SEE(ch, i))
        {
            if ((i->in_room != NOWHERE) &&
                ((GetMaxLevel(ch) >= LOW_IMMORTAL) || (real_roomp(i->in_room)->zone == real_roomp(ch->in_room)->zone)))
            {
...
int do_consider(struct char_data *ch, const char *argument, int cmd)
{
    ...
    diff = (GetMaxLevel(victim) + (GetSecMaxLev(victim) / 2) + (GetThirdMaxLev(victim) / 3)) -
           (GetMaxLevel(ch) + (GetSecMaxLev(ch) / 2) + (GetThirdMaxLev(ch) / 3)) + fuzz(MAX(1, GetMaxLevel(ch) / 5));
    if (diff <= -50)
    {
...
int do_spells(struct char_data *ch, const char *argument, int cmd)
{
    ...
    for (i = 1; i < MAX_SKILLS; i++)
    {
        ...
        scprintf(buf, MAX_STRING_LENGTH, "[%3d] %-20s  %2d %2d %2d %2d %2d %2d\r\n", i, spell_info[i].name,
                 spell_info[i].min_level[MAGE_LEVEL_IND], spell_info[i].min_level[CLERIC_LEVEL_IND],
                 spell_info[i].min_level[WARRIOR_LEVEL_IND], spell_info[i].min_level[THIEF_LEVEL_IND],
                 spell_info[i].min_level[RANGER_LEVEL_IND], spell_info[i].min_level[DRUID_LEVEL_IND]);
    }
    strlcat(buf, "\r\n", MAX_STRING_LENGTH);
    page_string(ch->desc, buf, 1);
    return TRUE;
...
void tick_line(struct char_data *ch, const char *label, int pulse_count)
{
    ...
    if (DEBUG)
        log_info("called %s with %s, %s, %d", __PRETTY_FUNCTION__, SAFE_NAME(ch), VNULL(label), pulse_count);
            ...
            else if (exitdata->to_room != NOWHERE)
            {
                if (IS_IMMORTAL(ch))
                {
                    snprintf(name[door], 21, "%c%c%c%c %-15.15s",
                             (IS_DARK(exitdata->to_room) || IS_DARKOUT(exitdata->to_room)) ? 'D' : ' ',
                             (IS_SET(exitdata->exit_info, EX_SECRET)) ? 'S' : ' ',
                             (IS_SET(exitdata->exit_info, EX_CLOSED)) ? 'C' : ' ',
                             (IS_SET(exitdata->exit_info, EX_LOCKED)) ? 'L' : ' ', real_roomp(exitdata->to_room)->name);
                    snprintf(vnum[door], 7, "#%-5.5d", exitdata->to_room);
                    snprintf(terrain[door], 13, sector_types[real_roomp(exitdata->to_room)->sector_type], "");
                }
                else
                {
                    if (!IS_SET(exitdata->exit_info, EX_SECRET))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                snprintf(name[door], 21, "%-20.20s", real_roomp(exitdata->to_room)->name);
                                snprintf(terrain[door], 13, sector_types[real_roomp(exitdata->to_room)->sector_type],
                                         "");
                            }
                        }
...
struct room_data *walk_room_path(struct room_data *this_room, const char *path)
{
    ...
    for (r = 0; r < (int)strlen(path); r++)
    {
        ...
        for (i = 0; i < (int)strlen(path); i++)
        {
            ...
            if (this_room->dir_option[dir] && this_room->dir_option[dir]->to_room >= 0 &&
