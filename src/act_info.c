...
void do_look(struct char_data *ch, char *argument, int cmd)
{
  ...
  if (GET_POS(ch) < POSITION_SLEEPING)
      ...
      case 5:
	{
	  ...
	  if ((exitp != NULL) &&
	      (IS_AFFECTED(ch, AFF_SCRYING) || IS_IMMORTAL(ch) || cmd == SKILL_PEER)) {
	    struct room_data                       *rp;
	 ...
	 */
	  ...
	  if (*arg2) {
		...
		if (tmp_object->obj_flags.value[1] <= 0) {
		  ...
		  temp =
		    ((tmp_object->obj_flags.value[1] * 3) / tmp_object->obj_flags.value[0]);
	  ...
	  if (*arg2) {
		...
		if (IS_SET(ch->specials.new_act, NEW_PLR_KILLOK)) {
		  if (IS_SET(tmp_char->specials.new_act, NEW_PLR_KILLOK)) {
		    ...
		    act("$n and $N share some private joke.", TRUE, ch, 0, tmp_char,
			TO_NOTVICT);
		  } else {
...
void do_ticks(struct char_data *ch, char *argument, int cmd)
{
  ...
  cprintf(ch, "  Next Zone tick:\t%1.2lf\n\r", pulse_zone / (double)PULSE_PER_SECOND);
  cprintf(ch, "  Next Teleport tick:\t%1.2lf\n\r", pulse_teleport / (double)PULSE_PER_SECOND);
  cprintf(ch, "  Next Nature tick:\t%1.2lf\n\r", pulse_nature / (double)PULSE_PER_SECOND);
  cprintf(ch, "  Next Violence tick:\t%1.2lf\n\r", pulse_violence / (double)PULSE_PER_SECOND);
  cprintf(ch, "  Next River tick:\t%1.2lf\n\r", pulse_river / (double)PULSE_PER_SECOND);
  cprintf(ch, "  Next Sound tick:\t%1.2lf\n\r", pulse_sound / (double)PULSE_PER_SECOND);
  cprintf(ch, "  Next Reboot tick:\t%1.2lf\n\r", pulse_reboot / (double)PULSE_PER_SECOND);
  cprintf(ch, "  Next Dump tick:\t%1.2lf\n\r", pulse_dump / (double)PULSE_PER_SECOND);
  ...
  for (door = 0; door < MAX_NUM_EXITS; door++) {
	...
	if (IS_IMMORTAL(ch)) {
	  sprintf(name[door], "%c%c%c%c %-15.15s",
		  ...
		  real_roomp(exitdata->to_room)->name);
	  sprintf(vnum[door], "#%-5.5d", exitdata->to_room);
	  sprintf(terrain[door], sector_types[real_roomp(exitdata->to_room)->sector_type], "");
	} else {
	  if (!IS_SET(exitdata->exit_info, EX_SECRET)) {
	    if (IS_SET(exitdata->exit_info, EX_CLOSED)) {
		...
		sprintf(name[door], "%-20.20s", real_roomp(exitdata->to_room)->name);
