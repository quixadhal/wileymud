...
{
  if (DEBUG > 2)
    log_info("called %s with %s, %s, %08zx, %08zx", __PRETTY_FUNCTION__, SAFE_NAME(ch), VNULL(arg), (size_t)equipment, (size_t)j);
...
void do_users(struct char_data *ch, const char *argument, int cmd)
{
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (flag)
      sprintf(line + strlen(line), "[%s@%s/%s]\r\n", d->username, d->host ? d->host: "unknown", d->ip ? d-> ip : "unknown");
    if (flag)
      strcat(buf, line);
  }
  cprintf(ch, "%s\r\n", buf);
