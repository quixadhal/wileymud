...
int 
GetItemClassRestrictions(struct obj_data *obj)
{
  int                              total = 0;
...
int 
CAN_SEE(struct char_data *s, struct char_data *o)
{
  if (!o)
    return (FALSE);
...
int 
exit_ok(struct room_direction_data *exit, struct room_data **rpp)
{
  struct room_data                *rp;
...
void 
Zwrite(FILE * fp, char cmd, int tf, int arg1, int arg2, int arg3,
       char *desc)
{
  char                             buf[100];
...
void 
RecZwriteObj(FILE * fp, struct obj_data *o)
{
  struct obj_data                 *t;
...
FILE                            *
MakeZoneFile(struct char_data * c)
{
  char                             buf[256];
  FILE                            *fp;
...
int 
IsImmune(struct char_data *ch, int bit)
{
...
int 
IsResist(struct char_data *ch, int bit)
{
  if (GetMaxLevel(ch) >= 50)
    return (1);
...
int 
IsSusc(struct char_data *ch, int bit)
{
...
int 
scan_number(char *text, int *rval)
{
  int                              length;
...
void 
sprintbit(unsigned long vektor, char *names[], char *result)
{
  long                             nr;
...
char 
in_group(struct char_data *ch1, struct char_data *ch2)
{
...
int 
getabunch(char *name, char *newname)
{
  int                              num = 0;
  char                             tmpname[80] = "\0";
...
int 
DetermineExp(struct char_data *mob, int exp_flags)
{
...
void 
RoomSave(struct char_data *ch, int start, int end)
{
  char                             fn[80],
                                   temp[2048],
                                   dots[500];
...
void 
RoomLoad(struct char_data *ch, int start, int end)
{
  FILE                            *fp;
  int                              rnum,
                                   i,
...
void 
fake_setup_dir(FILE * fl, int room, int dir)
{
  int                              tmp;
  char                             buf[256],
                                  *temp;
...
int 
IsHumanoid(struct char_data *ch)
{
/*
...
int 
IsLycanthrope(struct char_data *ch)
{
  switch (GET_RACE(ch)) {
  case RACE_LYCANTH:
    return (TRUE);
...
int 
IsDiabolic(struct char_data *ch)
{
  switch (GET_RACE(ch)) {
  case RACE_DEMON:
  case RACE_DEVIL:
...
int 
IsExtraPlanar(struct char_data *ch)
{
  switch (GET_RACE(ch)) {
  case RACE_DEMON:
  case RACE_DEVIL:
...
void 
SetHunting(struct char_data *ch, struct char_data *tch)
{
  int                              persist,
                                   dist;
  char                             buf[256];
...
void 
CallForGuard(struct char_data *ch, struct char_data *vict, int lev)
{
  struct char_data                *i;
...
void 
CallForAGuard(struct char_data *ch, struct char_data *vict, int lev)
{
  int                              zone;
...
void 
FighterMove(struct char_data *ch)
{
  int                              num;
...
void 
DevelopHatred(struct char_data *ch, struct char_data *v)
{
  int                              diff,
                                   patience,
                                   var;
...
int 
HasObject(struct char_data *ch, int ob_num)
{
  int                              j,
                                   found;
  struct obj_data                 *i;
...
int 
room_of_object(struct obj_data *obj)
{
...
struct char_data                *
char_holding(struct obj_data *obj)
{
  if (obj->in_room != NOWHERE)
    return NULL;
  else if (obj->carried_by)
...
int 
RecCompObjNum(struct obj_data *o, int obj_num)
{
...
void 
RestoreChar(struct char_data *ch)
{
...
void 
RemAllAffects(struct char_data *ch)
{
