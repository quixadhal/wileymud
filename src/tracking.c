    ...
    if (ch->persist <= 0)
    {
        ...
        else
        {
            if (ch->specials.hunting)
            {
                ...
                {
                    if (CanHate(ch, ch->specials.hunting) && (!IS_AFFECTED(ch->specials.hunting, AFF_HIDE)))
                    {
...
char *track_distance(struct char_data *ch, char *mob_name)
{
    ...
    if ((dir = choose_exit(this_room, dest_room, -MAX_ROOMS)) >= 0)
    {
        ...
        sprintf(buf, "(%3d%s, %3d%s, %3d%s)", abs(dx), dx < 0 ? "w" : "e", abs(dy), dy < 0 ? "s" : "n", abs(dz),
                dz < 0 ? "d" : "u");
    }
    return buf;
}
...
int do_immtrack(struct char_data *ch, const char *argument, int cmd)
{
    ...
    else
    {
        ...
        page_printf(ch,
                    "Path is %d rooms long, with a distance of (%d%s, %d%s, %d%s),\r\nwhich is roughly %d room lengths "
                    "away on a heading of %d degrees.\r\n",
