...
int GainLevel(struct char_data *master, struct char_data *ch, int class)
{
    ...
    if (DEBUG > 2)
	log_info("called %s with %s, %s, %d", __PRETTY_FUNCTION__, SAFE_NAME(master),
		 SAFE_NAME(ch), class);
	...
	if (GET_EXP(ch) >= titles[class][GET_LEVEL(ch, class) + 1].exp) {
		...
		if (GET_LEVEL(ch, class) < 4) {
		    ...
		    cprintf(ch,
			    "%s moans, 'I wish Quixadhal would let me TAX these youngsters...'\r\n",
			    NAME(master));
	    }
		...
		sprintf(buf,
			...
			NAME(ch), GET_LEVEL(ch, class), ordinal(GET_LEVEL(ch, class)));
...
int RangerGuildMaster(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (!*arg) {
	    sprintf(pagebuf, "You have got %d practice sessions left.\r\n", ch->specials.pracs);
	    sprintf(pagebuf + strlen(pagebuf),
		    "You can practice any of these woodsy skills:\r\n");
	    for (i = 0; i < MAX_SKILLS; i++)
		if (CanUseClass(ch, i, target) &&
		    (spell_info[i].min_level[target] <= GetMaxLevel(master) - 10) &&
		    (spell_info[i].min_level[target] >= GetMaxLevel(master) - 20)) {
...
int GenericGuildMaster(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (anumber != -1) {
	    percent_chance =
		ch->skills[r_skills[anumber].skill_numb].learned +
...
void exec_social(struct char_data *npc, const char *cmd, int next_line, int *cur_line,
		 void **thing)
{
    int                                     ok = FALSE;

    if (DEBUG > 3)
	log_info("called %s with %s, %s, %d, %08zx, %08zx", __PRETTY_FUNCTION__, SAFE_NAME(npc),
		 VNULL(cmd), next_line, (size_t) cur_line, (size_t) thing);
...
int RepairGuy(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (ITEM_TYPE(obj) == ITEM_ARMOR) {
		...
		act("$N says 'Well, I can't fix it, but I can make it last a little longer.'",
		    TRUE, ch, 0, vict, TO_CHAR);
		act("$N says 'That armor is old, you might want to get some new stuff I can fix later on.'", TRUE, ch, 0, vict, TO_ROOM);
		act("$N says 'That armor is old, you might want to get some new stuff I can fix later on.'", TRUE, ch, 0, vict, TO_CHAR);
	    }
		...
		if (GetMaxLevel(vict) > 25)		       /* super repair guy */
		    cost *= 2;
		if (cost > GET_GOLD(ch)) {
		    act("$N says 'I'm sorry, you don't have enough money.'", TRUE, ch, 0, vict,
			TO_ROOM);
		    act("$N says 'I'm sorry, you don't have enough money.'", TRUE, ch, 0, vict,
			TO_CHAR);
		} else {
...
int shylar_guard(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (ch->specials.fighting) {
		...
		act("$n shouts 'Help me Eli!  I don't get paid enough for THIS!!!'", TRUE, ch,
		    0, 0, TO_ROOM);
	    }
	    if (ch->specials.fighting)
		...
		if (IS_SET(i->obj_flags.wear_flags, ITEM_TAKE)) {
		    act("$n picks up something and puts it in his pocket.", FALSE, ch, 0, 0,
			TO_ROOM);
		    obj_from_room(i);
		    obj_to_char(i, ch);
		    return (TRUE);
	...
	if (ch->in_room != home_room) {
	    ...
	    if (0 <=
		(dir =
		 find_path(ch->in_room, is_target_room_p, (const void *)(size_t) home_room,
			   -200))) {
		go_direction(ch, dir);
		return TRUE;
	    } else {
	...
	if (the_path[path_index] < 0) {			       /* do special-emote */
		...
		case 1:
		    act("$n mumbles to Buck 'Strong coffee and a shot of anything.'", FALSE, ch,
			0, 0, TO_ROOM);
		...
		case 2:
		    ...
		    break;
		case 3:
		    act("$n says 'Eli, Why don't you get a REAL job?'", FALSE, ch, 0, 0,
			TO_ROOM);
		    act("$n shakes his head.", FALSE, ch, 0, 0, TO_ROOM);
		    break;
		case 4:
		    act("$n bellows 'Open up in there Tate!  I know you're up to something!'",
			FALSE, ch, 0, 0, TO_ROOM);
		    act("$n bangs on Tate's door until it nearly breaks down.", FALSE, ch, 0, 0,
			TO_ROOM);
		    break;
		case 5:
		    act("$n grabs Tate by the collar and shakes him.", FALSE, ch, 0, 0,
			TO_ROOM);
		    act("$n says 'Tate!  One day I'll catch you and your thieves...'", FALSE,
			ch, 0, 0, TO_ROOM);
		    act("WHAM!  $n slams Tate up against the wall.", FALSE, ch, 0, 0, TO_ROOM);
		    break;
		case 6:
		    act("$n snickers 'Hey there Ned!  You still playing with your nuts?'",
			FALSE, ch, 0, 0, TO_ROOM);
		    break;
		case 7:
		    act("$n says 'I'm glad your husband is out in the fields Maam.'", FALSE, ch,
			0, 0, TO_ROOM);
		    act("$n licks the Farmer's Wife.", FALSE, ch, 0, 0, TO_ROOM);
		    break;
		case 8:
		    act("$n murmers 'Let me show you some REAL magic Jessica.'", FALSE, ch, 0,
			0, TO_ROOM);
		    act("$n frenches Jessica and makes a charmed longstaff.", FALSE, ch, 0, 0,
			TO_ROOM);
		    break;
		case 9:
		    act("$n mumbles a prayer to Dread Quixadhal.", FALSE, ch, 0, 0, TO_ROOM);
		    act("$n slips a few coins from the Donation Pot.", FALSE, ch, 0, 0,
			TO_ROOM);
		    break;
		case 10:
		    act("$n exclaims 'Another hard day's work done!'", FALSE, ch, 0, 0,
			TO_ROOM);
		    act("$n orders several firebreathers and begins to down them.", FALSE, ch,
			0, 0, TO_ROOM);
		    break;
		case 11:
		    act("$n yawns mightily and passes out at the bar.", FALSE, ch, 0, 0,
			TO_ROOM);
		    GET_POS(ch) = POSITION_SLEEPING;
		    move = FALSE;
		    path_index = 0;
		...
		if (ch->in_room != the_path[path_index]) {
		    ...
		    if (0 <=
			(dir =
			 find_path(ch->in_room, is_target_room_p,
				   (const void *)(size_t) (the_path[path_index]), -200))) {
			go_direction(ch, dir);
			return TRUE;
		    } else {
			act("$n whimpers 'Help me!  I'm lost!.'", FALSE, ch, 0, 0, TO_ROOM);
...
int vampire(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (ch->specials.fighting && (ch->specials.fighting->in_room == ch->in_room)) {
	    cast_energy_drain(GetMaxLevel(ch), ch, "", SPELL_TYPE_SPELL, ch->specials.fighting,
			      0);
	}
	return TRUE;
    }
...
int cleric(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (OUTSIDE(ch) &&
	    (weather_info.sky >= SKY_RAINING) && (lspell >= 15) && (number(0, 3) == 0)) {
	...
	case 86 ... 93:
		...
		if (!IS_IMMORTAL(i) && !number(0, (IS_PC(i) ? 5 : 50))) {
			...
			case SEX_FEMALE:
			    ...
			    break;
			default:
			    sprintf(buf, "Be at ease %s, whatever you are!",
				    fname(GET_NAME(i)));
			    break;
		    }
		    do_shout(ch, buf, 0);
	...
	case 94 ... 97:
		...
		if (!IS_IMMORTAL(i) && !number(0, (IS_PC(i) ? 5 : 50))) {
			...
			case SEX_FEMALE:
			    ...
			    break;
			default:
			    sprintf(buf, "I shall HEAL you %s, whatever you are!",
				    fname(GET_NAME(i)));
			    break;
		    }
		    do_shout(ch, buf, 0);
...
int temple_labrynth_sentry(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (GetMaxLevel(tch) > 10 && CAN_SEE(ch, tch)) {
	    ...
	    act("The sentry looks concerned and continues to push you away", 1, ch, 0, 0,
		TO_ROOM);
...
int NudgeNudge(struct char_data *ch, int cmd, const char *arg)
{
	...
	case NN_LOOSE:
		...
		&& (number(0, 99) < 50)) {
		act("$n falls and grovels in abject terror before you.", FALSE, ch, 0, 0,
		    TO_CHAR);
		act("$n falls and grovels in abject terror before $N.", FALSE, ch, 0, vict,
		    TO_ROOM);
		return FALSE;
	    }
	    /*
	...
	case NN_FOLLOW:
		...
		case 0:
			...
			act("$n looks around for something it thinks is missing.", FALSE, ch, 0,
			    0, TO_ROOM);
		    }
		    break;
		case 1:
		...
		case 3:
			...
			act("$n asks you, 'If a dragon held me captive, would you save me?'",
			    FALSE, ch, 0, 0, TO_CHAR);
			act("$n asks $N, 'If a dragon held me captive, would you save me?'",
			    FALSE, ch, 0, ch->master, TO_ROOM);
		    } else {
		...
		case 5:
			...
			act("$n says to $N, '$N, I love you!'", FALSE, ch, 0, ch->master,
			    TO_ROOM);
		    } else {
		    }
		    break;
...
void zm_init_combat(struct char_data *zmaster, struct char_data *target)
{
    ...
    if (DEBUG > 3)
	log_info("called %s with %s, %s", __PRETTY_FUNCTION__, SAFE_NAME(zmaster),
		 SAFE_NAME(target));
...
int zm_kill_aggressor(struct char_data *zmaster)
{
	...
	if (fwr->follower->specials.fighting &&
	    ...
	    if ((GetMaxLevel(fwr->follower->specials.fighting) <=
		 ((maxlevel = GetMaxLevel(zmaster)) / 4))
		&& (fwr->follower->specials.fighting->in_room == zmaster->in_room)) {
...
int zm_kill_wimps(struct char_data *zmaster)
{
	...
	if (vict->master != zmaster && vict != zmaster
	    && ((mlev = GetMaxLevel(vict)) < maxlevel)
	    && (GET_POS(vict) > POSITION_DEAD) && (number(0, 99) < 65)) {
	    if ((real_roomp(vict->in_room)->zone == 10) && IS_PC(vict) && (mlev < 4))
		continue;
	    if (IS_REALLY_HOLY(vict)) {
		if (mlev <= (maxlevel - 2)) {
		    act("$n howls 'BEGONE foul creature of Light!'", FALSE, zmaster, 0, 0,
			TO_ROOM);
		    ...
		    act("$n glares at $N with unimaginable horror!", TRUE, zmaster, 0, vict,
			TO_ROOM);
		...
		if (mlev <= ((maxlevel / 2) + (maxlevel / 4))) {
		    ...
		    act("$n glares at $N with undisguised hatred.", TRUE, zmaster, 0, vict,
			TO_ROOM);
		...
		act("$n nods slightly to $N and continues her work.", TRUE, zmaster, 0, vict,
		    TO_ROOM);
...
int zombie_master(struct char_data *ch, int cmd, const char *arg)
{
	...
	case POSITION_STANDING:
	    ...
	    temp1 =
		get_obj_in_list_vis(zmaster, "corpse", real_roomp(zmaster->in_room)->contents);
		...
		if (x > 50) {
			...
			REMOVE_BIT(ack->follower->specials.affected_by,
				   AFF_CHARM | AFF_GROUP | AFF_FLYING);
#if 0
			stop_follower(ack);
#endif
...
int pray_for_items(struct char_data *ch, int cmd, const char *arg)
{
	...
	for (ext = tmp_obj->ex_description; ext; ext = ext->next)
		...
		if (gold == 0) {
			...
			FALSE, ch, 0, 0, TO_ROOM);
		    act("You notice a faint spark in Quixadhal's eye.", FALSE, ch, 0, 0,
			TO_CHAR);
		}
...
int jugglernaut(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (random() % 6) {
	    if (random() % 2) {
		act("$n tosses $p high into the air and catches it.", TRUE, ch, tmp_obj, NULL,
		    TO_ROOM);
	    } else {
...
int eli_priest(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (!IS_IMMORTAL(vict)) {
	    if (IsUndead(vict)) {
		act("$n points at $N and bellows, 'BEGONE Abomination! Flee back to Dread Quixadhal, your dark master!'", TRUE, ch, 0, vict, TO_ROOM);
		say_spell(ch, SPELL_FLAMESTRIKE);
	...
	if ((GET_POS(vict) != POSITION_FIGHTING) &&
	    (GET_POS(vict) != POSITION_SLEEPING) && !IS_IMMORTAL(vict)) {
		...
		if ((GET_ALIGNMENT(vict) > ALIGN_VERY_EVIL) ||
		    ...
		    act("$n says, You are hurt $N, let me heal you.", TRUE, ch, 0, vict,
			TO_ROOM);
		    is_evil = FALSE;
		...
		if ((GET_ALIGNMENT(vict) > ALIGN_VERY_EVIL) ||
		    ((GET_ALIGNMENT(vict) > ALIGN_VILE) && (GetMaxLevel(vict) < 4))) {
		    act("$n says, You seem tired $N, let me help you.", TRUE, ch, 0, vict,
			TO_ROOM);
		    is_evil = FALSE;
		...
		if ((GET_ALIGNMENT(vict) > ALIGN_VERY_EVIL) ||
		    ((GET_ALIGNMENT(vict) > ALIGN_VILE) && (GetMaxLevel(vict) < 4))) {
		    act("$n says, You seem to be blind $N, let me cure you.", TRUE, ch, 0, vict,
			TO_ROOM);
		    is_evil = FALSE;
		...
		if ((GET_ALIGNMENT(vict) > ALIGN_VERY_EVIL) ||
		    ((GET_ALIGNMENT(vict) > ALIGN_VILE) && (GetMaxLevel(vict) < 4))) {
		    act("$n says, You seem to be sick $N, let me cure you.", TRUE, ch, 0, vict,
			TO_ROOM);
		    is_evil = FALSE;
	...
	switch (number(0, 20)) {
	    ...
	    case 2:
	    case 3:
		act("$n says, I heard the lands were getting tough, so I decide to move a church here.", TRUE, ch, 0, 0, TO_ROOM);
		break;
	    case 4:
	    case 5:
...
int mosquito(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (bloody->in_room == ch->in_room) {
		...
		if (number(0, 99) < 10) {
		    ...
		    act("$n anticipates the taste of $N's immortal blood!", TRUE, ch, 0, bloody,
			TO_ROOM);
		    return FALSE;
		}
	    }
...
int BerserkerAxe(struct char_data *ch, int cmd, const char *arg)
{
	...
	if (IS_IMMORTAL(ch))
	    return 0;
	act("$n looks abashed and then seems taken by a sudden fury!", TRUE, ch, 0, ch,
	    TO_ROOM);
	...
	if (number(0, 99) < 50) {
	    act("$n jumps up and stares all around with a savage grin!", TRUE, ch, 0, ch,
		TO_ROOM);
...
{
    if (DEBUG > 3)
	log_info("called %s with %s, %s", __PRETTY_FUNCTION__, SAFE_NAME(master),
		 SAFE_NAME(vict));
	...
	if (IS_GOOD(vict)) {
	    ...
	    cprintf(vict,
		    "%s sneers, 'You are a fool.  Go find another fool to teach you!'\r\n",
		    NAME(master));
	}
...
void gm_wrong_alignment(struct char_data *master, struct char_data *vict)
{
	...
	if (IS_VERY_GOOD(vict)) {
	    act("$n screams 'How dare you insult my presence, DIE!'", FALSE, master, 0, 0,
		TO_ROOM);
	    ...
	    cprintf(vict,
		    "%s smirks, 'Simpering fool!  Begone, ere I feast upon your heart!'\r\n",
		    NAME(master));
	}
	...
	if (IS_VERY_EVIL(vict)) {
	    act("$n cries 'How dare you defile this place, REPENT!'", FALSE, master, 0, 0,
		TO_ROOM);
	    hit(master, vict, TYPE_UNDEFINED);
	} else if (IS_EVIL(vict)) {
	    ...
	    cprintf(vict,
		    "%s says, 'I cannot help you until you repent and reject darkness.'\r\n",
		    NAME(master));
	}
    } else if (IS_GOOD(master)) {
	if (IS_EVIL(vict)) {
...
void gm_prac(struct char_data *master, struct char_data *vict, int target, const char *arg)
{
    ...
    if (DEBUG > 3)
	log_info("called %s with %s, %s, %d, %s", __PRETTY_FUNCTION__, SAFE_NAME(master),
		 SAFE_NAME(vict), target, VNULL(arg));
...
int k_kill_wimps(struct char_data *karrn)
{
	...
	count++;
    if (count > 1) {					       /* Karrn himself is always present */
	...
	for (vict = real_roomp(karrn->in_room)->people; vict; vict = vict->next_in_room) {
		...
		if (!str_cmp(GET_NAME(vict), "Quixadhal")) {
			...
			switch (choice) {
			    ...
			    case 3:
			    case 4:
				act("$n dances around $N and yips with delight!", TRUE, karrn,
				    0, vict, TO_ROOM);
				break;
			    case 5:
			    case 6:
			    case 7:
				act("$N laughs as $n buffets him with his wings.", TRUE, karrn,
				    0, vict, TO_ROOM);
				break;
			    case 8:
				act("$N waves to $n, who hops out and brings back a dark beer.",
				    TRUE, karrn, 0, vict, TO_ROOM);
				break;
			    case 9:
			...
			switch (choice) {
			    case 0:
				act("$n retches and gags as $N forgot to bathe.", TRUE, karrn,
				    0, vict, TO_ROOM);
				break;
			    case 1:
				act("$n farts...the smell is hideous!", TRUE, karrn, 0, vict,
				    TO_ROOM);
			    ...
			    case 4:
			    case 5:
				act("$n laughs, \"Mock ME will you Muidnar?\"", TRUE, karrn, 0,
				    vict, TO_ROOM);
			    ...
			    case 6:
			    case 7:
				act("$n lifts his leg and pisses on $N.", TRUE, karrn, 0, vict,
				    TO_ROOM);
				break;
			    case 8:
				act("$n coughs up a huge hairball on $N.", TRUE, karrn, 0, vict,
				    TO_ROOM);
				break;
			    case 9:
				act("$n grovels before $N!", TRUE, karrn, 0, vict, TO_ROOM);
		    ...
		    act("$n hisses 'Disturb me NOT!  I shall eat now foolish mortal.'", FALSE,
			karrn, 0, 0, TO_ROOM);
...
int Karrn(struct char_data *ch, int cmd, const char *arg)
{
	...
	case POSITION_STANDING:
		...
		if (number(0, 99) < 10) {
		    for (i = character_list; i; i = i->next) {
			if (IS_PC(i) && IS_IMMORTAL(i) && i != karrn
			    && i->in_room != karrn->in_room) {
			    if (!str_cmp(GET_NAME(i), "Muidnar")) {
				tmp = i;
				target = karrn->in_room;
				do_shout(karrn, "Muidnar!  Get over here you BASTARD!", 0);
				act("A red slice of fire opens before $n, who is enveloped screaming, then silence.", TRUE, tmp, 0, 0, TO_ROOM);
				if (MOUNTED(tmp)) {
				    char_from_room(tmp);
				    char_from_room(MOUNTED(tmp));
				    char_to_room(tmp, target);
				    char_to_room(MOUNTED(tmp), target);
				    act("$n emerges from a column of fire that Karrn dismisses with a flick of his tail.", FALSE, tmp, 0, 0, TO_ROOM);
				    if (IS_PC(tmp))
					do_look(tmp, "", 15);
				    if (IS_PC(tmp))
