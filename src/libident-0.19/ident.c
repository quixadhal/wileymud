...
IDENT *ident_lookup __P2(int, fd, int, timeout)
{
    ...
    return ident_query(&localaddr.sin_addr, &remoteaddr.sin_addr, ntohs(localaddr.sin_port), ntohs(remoteaddr.sin_port),
                       timeout);
}

IDENT *ident_query __P5(struct in_addr *, laddr, struct in_addr *, raddr, int, lport, int, rport, int, timeout)
{
    int res;
    ident_t *id;
    ...
    if (timeout)
        ...
        res = id_parse(id, (struct timeval *)0, &ident->lport, &ident->fport, &ident->identifier, &ident->opsys,
                       &ident->charset);
...
char *ident_id __P2(int, fd, int, timeout)
{
    IDENT *ident;
    char *id = 0;
