...
void imc_write_buffer( const char *txt )
{
    ...
    * Copy.
    */
   strncpy( this_imcmud->outbuf + this_imcmud->outtop, output, outlen );   /* Leave this one alone! BAD THINGS(TM) will happen if you don't! */
...
IMC_CMD( imclist )
{
   ...
   for( p = first_rinfo; p; p = p->next, count++ )
   {
      ...
      *serverpath = *netname = '\0';
      ...
      imcpager_printf( ch, "~%c%-20.20s ~Y%-15.15s %-5.5s ~B%-20.20s ~g%-15.15s ~G%s~!\r\n",
                       p->expired ? 'R' : 'c', p->name, p->host, p->port, p->version, netname, serverpath );
   }
