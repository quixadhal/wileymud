...
const char *const imcperm_names[] = {"Notset", "None", "Mort", "Imm", "Admin", "Imp"};
...
const char *const imc_color_symbols[] = {
    ...
    "~R", "~G", "~M", "~z", "^B~W", "^r~Y"
    // ichat
    // ~rdarkred~gdarkgreen~Yyellow~Bblue~Wwhite~mdarkmagenta~Ccyan~Rred~Ggreen~Mmagenta~zgrey^B~Wwhiteonblue^r~Yyellowonred~!
    // 13 colors before it loops. ichat
    // ~gdark~Ggreen~rdark~Rred~ydark~Yyellow~mdark~Mmagenta~wdark~Wwhite~cdark~Ccyan~bdark~Bblue~xdark~zblack~!
};
...
void update_imchistory(IMC_CHANNEL *channel, char *message)
{
    ...
    for (x = 0; x < MAX_IMCHISTORY; x++)
    {
        if (channel->history[x] == NULL)
        {
            ...
            snprintf(buf, LGST, "~R[%-2.2d/%-2.2d %-2.2d:%-2.2d] ~G%s", local->tm_mon + 1, local->tm_mday,
...
void imc_process_finger(const char *from, const char *type)
{
    ...
    snprintf(buf, IMC_BUFF_SIZE,
             ...
             CH_IMCNAME(victim), (IMCAFK(victim) ? "AFK" : "Lurking about"), imcperm_names[IMCPERM(victim)],
    ...
    if (!IMCIS_SET(IMCFLAG(victim), IMC_PRIVACY))
        snprintf(buf + strlen(buf), IMC_BUFF_SIZE - strlen(buf),
                 ...
                 IMC_ICQ(victim), (IMC_AIM(victim) && IMC_AIM(victim)[0] != '\0') ? IMC_AIM(victim) : "None",
...
void imc_parse_packet(const char *packet)
{
    ...
    if (!pfun)
    {
        if (imcpacketdebug)
        {
            imclog("PACKET: From %s, Seq %lu, Route %s, Type %s, To %s, EXTRA %s", p->from, seq, p->route, p->type,
                   p->to, packet);
    ...
    for (;;)
    {
        ...
        case 'E':
            if (!strcasecmp(word, "End"))
            {
                ...
                snprintf(lbuf1, LGST, "%s %s.%s", CODEBASE_VERSION_TITLE, CODEBASE_VERSION_MAJOR,
                         CODEBASE_VERSION_MINOR);
                if (this_imcmud->base)
                    IMCSTRFREE(this_imcmud->base);
...
bool imc_read_config(int desc)
{
    ...
    for (;;)
    {
        ...
        if (!strcasecmp(word, "IMCCONFIG") && this_imcmud == NULL)
        {
            ...
            /*
             * If someone can think of better default values, I'm all ears. Until then, keep your bitching to
             * yourselves.
             */
...
bool imc_server_connect(void)
{
    ...
    if (this_imcmud->sha256)
    {
         ...
         * * 2. You are connecting to an older server which does not support it, so you will continue connecting this
         * way.
        ...
        if (!this_imcmud->sha256pass)
        {
            snprintf(buf, LGST, "PW %s %s version=%d autosetup %s SHA256", this_imcmud->localname,
                     this_imcmud->clientpw, IMC_VERSION, this_imcmud->serverpw);
        }
        /*
     ...
     */
        ...
        snprintf(buf, LGST, "PW %s %s version=%d autosetup %s", this_imcmud->localname, this_imcmud->clientpw,
...
IMC_CMD(imcchanlist)
{
    ...
    for (c = first_imc_channel; c; c = c->next)
    {
        ...
        imc_printf(ch, "~c%-15.15s ~%c%-*.*s ~B%-15.15s ~b%-7s %s\r\n", c->name, col, c->local_name ? 15 : 17,
                   c->local_name ? 15 : 17, c->local_name ? c->local_name : "~Y(not local)  ", c->owner,
...
IMC_CMD(imclist)
{
    ...
    for (p = first_rinfo; p; p = p->next, count++)
    {
        ...
        *serverpath = *netname = '\0';
        ...
        imcpager_printf(ch, "~%c%-20.20s ~Y%-15.15s %-5.5s ~B%-20.20s ~g%-15.15s ~G%s~!\r\n", p->expired ? 'R' : 'c',
                        p->name, p->host, p->port, p->version, netname, serverpath);
    }
...
const char *imc_find_social(CHAR_DATA *ch, const char *sname, const char *person, const char *mud, int victim)
{
    ...
    {
        if (person && person[0] != '\0' && !strcasecmp(person, CH_IMCNAME(ch)) && mud && mud[0] != '\0' &&
...
char *imc_act_string(const char *format, CHAR_DATA *ch, CHAR_DATA *vic)
{
    ...
    while (*format != '\0')
    {
        ...
        else
        {
            ...
            case 'e':
                i = should_upper ? imccapitalize(he_she[URANGE(0, CH_IMCSEX(ch), 2)])
                                 : he_she[URANGE(0, CH_IMCSEX(ch), 2)];
                break;

            case 'E':
                i = should_upper ? imccapitalize(he_she[URANGE(0, CH_IMCSEX(vic), 2)])
                                 : he_she[URANGE(0, CH_IMCSEX(vic), 2)];
                break;

            case 'm':
                i = should_upper ? imccapitalize(him_her[URANGE(0, CH_IMCSEX(ch), 2)])
                                 : him_her[URANGE(0, CH_IMCSEX(ch), 2)];
                break;

            case 'M':
                i = should_upper ? imccapitalize(him_her[URANGE(0, CH_IMCSEX(vic), 2)])
                                 : him_her[URANGE(0, CH_IMCSEX(vic), 2)];
                break;

            case 's':
                i = should_upper ? imccapitalize(his_her[URANGE(0, CH_IMCSEX(ch), 2)])
                                 : his_her[URANGE(0, CH_IMCSEX(ch), 2)];
                break;

            case 'S':
                i = should_upper ? imccapitalize(his_her[URANGE(0, CH_IMCSEX(vic), 2)])
