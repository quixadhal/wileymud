...
void show_bans(struct char_data *ch)
{
    ...
    for (int i = 0; i < ban_list_count; i++)
    {
        if (!strcasecmp(ban_list[i].ban_type, "NAME") || !strcasecmp(ban_list[i].ban_type, "IP"))
            visible_bans++;
    }
    cprintf(ch, "Ban List:\r\n");
    if (visible_bans < 1)
    ...
    {
        cprintf(ch, "%-10s %4s %-16s %-16s %s\r\n", "Date Set", "Type", "Name/IP", "Set By", "Reason");
        cprintf(ch, "%-10s %4s %-16s %-16s %s\r\n", "----------", "----", "----------------", "----------------",
                "----------------------------");
        for (int i = 0; i < ban_list_count; i++)
        {
            if (!strcasecmp(ban_list[i].ban_type, "NAME"))
                cprintf(ch, "%-10s %4s %-16s %-16s %s\r\n", ban_list[i].updated_text, "NAME", ban_list[i].name,
                        ban_list[i].set_by, ban_list[i].reason);
        }
        for (int i = 0; i < ban_list_count; i++)
        {
            if (!strcasecmp(ban_list[i].ban_type, "IP"))
                cprintf(ch, "%-10s %4s %-16s %-16s %s\r\n", ban_list[i].updated_text, "IP", ban_list[i].ip,
                        ban_list[i].set_by, ban_list[i].reason);
        }
    }
}
