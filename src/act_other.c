...
void do_junk(struct char_data *ch, char *argument, int cmd)
{
  ...
  struct obj_data *tmp_object,*old_object;
  int num, p, count;
if(DEBUG) dlog("do_junk");
/*
...
void do_practice(struct char_data *ch, char *arg, int cmd) 
{
  char buf[256];
  int i;
  char first_arg[100],sec_arg[100];  
  int flag;
...
/*    {"listen",		SKILL_DETECT_NOISE}, 
  ...
  case 'm':
	...
	return;
      ...
      flag =0;
      if(is_abbrev(sec_arg,"known")) flag = 1;
...
do_plr_noteleport(struct char_data *ch, char *argument, int cmd)
{
  ...
  if(DEBUG) dlog("do_plr_noteleport");
  if (IS_NPC(ch)) return;
