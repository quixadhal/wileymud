...
void do_sneak(struct char_data *ch, const char *argument, int cmd)
{
    ...
    if (percent_chance >
	ch->skills[SKILL_SNEAK].learned + dex_app_skill[(int)GET_DEX(ch)].sneak)
	return;
...
void do_steal(struct char_data *ch, const char *argument, int cmd)
{
	...
	if (AWAKE(victim) && (percent_chance > ch->skills[SKILL_STEAL].learned)) {
	    ohoh = TRUE;
	    act("Oops..", FALSE, ch, 0, 0, TO_CHAR);
	    act("You discover that $n has $s hands in your wallet.", FALSE, ch, 0, victim,
		TO_VICT);
...
void do_group(struct char_data *ch, const char *argument, int cmd)
{
	...
	if (found) {
		...
		if (IS_IMMORTAL(victim) && !IS_IMMORTAL(ch)) {
		    act("You really don't want $n in your group.", FALSE, victim, 0, 0,
			TO_CHAR);
		    return;
		}
/*
 ...
 */
		...
		if (IS_PC(victim) && victim != ch) {
		    ...
		    if (victlvl >= toohigh) {
			act("You WISH $N would deign to join your group!", FALSE, ch, 0, victim,
			    TO_CHAR);
			act("$N BEGS you to help $M go kill the chickens.", FALSE, victim, 0,
			    ch, TO_CHAR);
			act("$N grovels and begs $n to join $S group.", FALSE, victim, 0, ch,
			    TO_ROOM);
			return;
		    }
		}
		if (victim != ch) {
		    act("$n joins the glorious cause of $N's group.", FALSE, victim, 0, ch,
			TO_ROOM);
...
void do_use(struct char_data *ch, const char *argument, int cmd)
{
	...
	if (bits) {
		...
		((*spellp->spell_pointer)
		 ((char)stick->obj_flags.value[0], ch, "", SPELL_TYPE_WAND, tmp_char,
		  tmp_object));
		WAIT_STATE(ch, PULSE_VIOLENCE);
	    } else {
