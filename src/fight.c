...
void change_alignment(struct char_data *ch, struct char_data *victim)
{
    ...
    {
        GET_ALIGNMENT(ch) -= (GET_ALIGNMENT(victim) / 200) * (MAX(1, GetMaxLevel(ch) - GetMaxLevel(victim)));
    }
    else if (IS_EVIL(ch) && (IS_GOOD(victim)))
    {
        GET_ALIGNMENT(ch) -= (GET_ALIGNMENT(victim) / 300) * (MAX(1, GetMaxLevel(ch) - GetMaxLevel(victim)));
    }
    else if ((IS_GOOD(ch)) && IS_EVIL(victim))
    {
        GET_ALIGNMENT(ch) -= (GET_ALIGNMENT(victim) / 400) * (MAX(1, GetMaxLevel(ch) - GetMaxLevel(victim)));
    }
    else if (IS_EVIL(ch) && (IS_EVIL(victim)))
    {
        GET_ALIGNMENT(ch) -= (GET_ALIGNMENT(victim) / 450) * (MAX(1, GetMaxLevel(victim) - GetMaxLevel(ch)));
    }
    else
    {
        GET_ALIGNMENT(ch) -= (GET_ALIGNMENT(victim) / 200) * (MAX(1, GetMaxLevel(victim) - GetMaxLevel(ch)));
    }
...
void group_gain(struct char_data *ch, struct char_data *victim)
{
    ...
    { /* if they are grouped, they suck exp */
    ...
    if (member_count == 1)
    {
        ...
        if (IS_SET(k->specials.new_act, NEW_PLR_KILLOK))
            amnt *= 1.10;
#ifdef MOB_LEVELING
        if (IS_NPC(k) && (!IS_SET(k->specials.act, ACT_POLYSELF) && !IS_SET(k->specials.act, ACT_POLYSELF)))
            amnt /= 4.0;
#endif
    ...
    if (IS_AFFECTED(k, AFF_GROUP))
    {
        amnt = share * ((double)GetMaxLevel(k) + ((double)GetSecMaxLev(k) / 2.0) + ((double)GetThirdMaxLev(k) / 4.0));
        leftover += share * (((double)GetSecMaxLev(k) / 2.0) + (3.0 * ((double)GetThirdMaxLev(k) / 4.0)));
        if (IS_SET(k->specials.new_act, NEW_PLR_KILLOK))
            amnt *= 1.10;
    ...
    for (f = k->followers; f; f = f->next)
    {
        if (IS_AFFECTED(f->follower, AFF_GROUP))
        {
            amnt = share * ((double)GetMaxLevel(f->follower) + ((double)GetSecMaxLev(f->follower) / 2.0) +
            ...
            if (f->follower->in_room != ch->in_room)
            {
                amnt /= 10.0;
#ifdef MOB_LEVELING
                if (IS_NPC(f->follower) && (!IS_SET(f->follower->specials.act, ACT_POLYSELF) &&
    ...
    if (k->in_room != ch->in_room)
        amnt /= 10.0;
    if (amnt > 0.0 && (IS_PC(k) || IS_SET(k->specials.act, ACT_POLYSELF) || IS_SET(k->specials.act, ACT_POLYSELF)))
    {
    ...
    for (f = k->followers; f; f = f->next)
    {
        ...
        if (f->follower->in_room != ch->in_room)
            amnt /= 10.0;
        if (amnt > 0.0 &&
            ((IS_PC(f->follower) && IS_AFFECTED(f->follower, AFF_GROUP)) ||
             IS_SET(f->follower->specials.act, ACT_POLYSELF) || IS_SET(f->follower->specials.act, ACT_POLYSELF)))
        {
...
int damage(struct char_data *ch, struct char_data *victim, int dam, int attacktype)
{
    ...
    if (DEBUG > 2)
        log_info("called %s with %s, %s, %d, %d", __PRETTY_FUNCTION__, SAFE_NAME(ch), SAFE_NAME(victim), dam,
                 attacktype);
    ...
    if (IS_NPC(ch) && IS_NPC(victim) && victim->master && !number(0, 10) && IS_AFFECTED(victim, AFF_CHARM) &&
        ...
        if (IS_NPC(victim) || victim->desc)
        {
            ...
            else
            {
                ...
                if (IS_SET(ch->specials.new_act, NEW_PLR_KILLOK))
                    experience *= 1.10;
#ifdef MOB_LEVELING
                if (IS_NPC(ch) && (!IS_SET(ch->specials.act, ACT_POLYSELF) && !IS_SET(ch->specials.act, ACT_POLYSELF)))
                    experience /= 4;
#endif
...
void hit(struct char_data *ch, struct char_data *victim, int type)
{
    ...
    if (ch->in_room != victim->in_room)
    {
        log_info("NOT in same room when fighting : %s, %s", ch->player.name, victim->player.name);
        stop_fighting(ch);
        return;
    }
    if (!IS_IMMORTAL(ch))
    ...
    else if (ch->equipment[WIELD_TWOH] && (ch->equipment[WIELD_TWOH]->obj_flags.type_flag == ITEM_WEAPON))
    {
#if 0
	two_handed = 1;
#endif
            ...
            if (AWAKE(ch) && (ch->in_room == ch->specials.fighting->in_room) && (!IS_AFFECTED(ch, AFF_PARALYSIS)))
            {
                ...
                if ((IS_PC(ch)) && (HasClass(ch, CLASS_RANGER) || HasClass(ch, CLASS_WARRIOR)))
                {
                    if (((GET_LEVEL(ch, WARRIOR_LEVEL_IND) > 6) && (ch->mult_att)) ||
                        ...
                        (GET_LEVEL(ch, WARRIOR_LEVEL_IND) > 12) || (GET_LEVEL(ch, RANGER_LEVEL_IND) > 12))
                    {
                        if (ch->specials.fighting)
...
struct char_data *FindVictim(struct char_data *ch)
{
    ...
    total = (fjump * ftot) + (cjump * ctot) + (tjump * ttot) + (njump * ntot) + (mjump * mtot) + (rjump * rtot) +
            (djump * dtot);
...
void DamageStuff(struct char_data *v, int type, int dam)
{
    ...
    if (type >= TYPE_HIT && type <= TYPE_SMITE)
    {
        ...
        if (v->equipment[num])
        {
            if ((type == TYPE_BLUDGEON && dam > 10) || (type == TYPE_CRUSH && dam > 8) ||
                (type == TYPE_SMASH && dam > 10) || (type == TYPE_BITE && dam > 15) ||
                (type == TYPE_CLAW && dam > 20) || (type == TYPE_SLASH && dam > 20) ||
