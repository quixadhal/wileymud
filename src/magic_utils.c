...
void AreaEffectSpell(struct char_data *castor, int dam, int spell_type, int zflag, const char *zone_mesg)
{
    ...
    if (DEBUG > 2)
        log_info("called %s with %s, %d, %d, %d, %s", __PRETTY_FUNCTION__, SAFE_NAME(castor), dam, spell_type, zflag,
                 VNULL(zone_mesg));
    ...
    for (victim = character_list; victim; victim = next_victim)
    {
        ...
        if ((castor->in_room == victim->in_room) && castor != victim)
        {
            ...
            if (IS_AFFECTED(victim, AFF_GROUP))
                if ((victim == castor->master) || (victim->master == castor) || (victim->master == castor->master))
                    continue;
            switch (spell_type)
            {
