...
void reset_colors( CHAR_DATA * ch )
{
   ...
   if( !IS_NPC( ch ) )
   {
      ...
      log_info( "%s: Attempting to reset NPC colors: %s", __FUNCTION__, ch->short_descr );
}
   ...
   if( txt && *txt && d )
   {
      ...
      while( ( colstr = strpbrk( prevstr, "&^}hH" ) ) != NULL )
      {
         ...
         if( colstr > prevstr )
         {
            ...
            {
               log_error( "%s: OVERFLOW in internal MAX_STRING_LENGTH buffer!", __PRETTY_FUNCTION__ );
               break;
            }
...
void set_char_color( short AType, CHAR_DATA * ch )
{
   ...
   if( !ch->desc )
   {
      log_error( "set_char_color: NULL descriptor after WTB! CH: %s", ch->name ? ch->name : "Unknown?!?" );
      return;
   }
...
void write_to_pager( DESCRIPTOR_DATA * d, const char *txt, unsigned int length )
{
   ...
   while( d->pagetop + length >= d->pagesize )
   {
      ...
      {
         log_error( "%s", "Pager overflow.  Ignoring.\r\n" );
         d->pagetop = 0;
         d->pagepoint = NULL;
         DISPOSE( d->pagebuf );
         d->pagesize = MAX_STRING_LENGTH;
...
void set_pager_color( short AType, CHAR_DATA * ch )
{
   ...
   if( !ch->desc )
   {
      log_error( "set_pager_color: NULL descriptor after WTP! CH: %s", ch->name ? ch->name : "Unknown?!?" );
      return;
   }
   ch->desc->pagecolor = ch->colors[AType];
}
...
void send_to_desc_color( const char *txt, DESCRIPTOR_DATA * d )
{
   if( !d )
   {
      log_error( "%s: NULL *d", __FUNCTION__ );
      return;
   }
...
void send_to_char( const char *txt, CHAR_DATA * ch )
{
   if( !ch )
   {
      log_error( "%s", "send_to_char: NULL ch!" );
      return;
   }
...
void send_to_pager( const char *txt, CHAR_DATA * ch )
{
   if( !ch )
   {
      log_error( "%s", "send_to_pager: NULL ch!" );
      return;
   }
