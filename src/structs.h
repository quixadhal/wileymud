...
#define MAX_DESC_LENGTH		1600	/*
					 * 80 chars, 20 rows 
					 */
#define MAX_MY_STRING_LENGTH  	80	/*
					 * 80 chars, 1 row 
					 */
...
#define MAIL_PATH "mail"	/*
				 * this is offset by the location of lib 
				 */
...
typedef struct {
  ...
  int                              sex;		/*
						 * number 1=male,2=female,3=both,4=neut,5=m&n,6=f&n,7=all
						 */
  int                              race;	/*
						 * number 
						 */
...
#define ITEM_METAL           4	/*
				 * undefined...  
				 */
#define ITEM_MINERAL         8	/*
				 * undefined?    
				 */
#define ITEM_ORGANIC        16	/*
				 * undefined?    
				 */
...
#define ITEM_ANTI_GOOD     512	/*
				 * not usable by good people    
				 */
#define ITEM_ANTI_EVIL    1024	/*
				 * not usable by evil people    
				 */
#define ITEM_ANTI_NEUTRAL 2048	/*
				 * not usable by neutral people 
				 */
...
struct obj_data {
  ...
  struct char_data                *carried_by;	/*
						 ...
						 */
  byte                             eq_pos;	/*
						 * what is the equip. pos?          
						 */
  struct char_data                *equipped_by;		/*
							 * equipped by :NULL in room/conta  
							 */
...
#define NOWHERE    -1		/*
				 * nil reference for room-database    
				 */
...
struct room_data {
  ...
  int                              river_dir;	/*
						 * dir of flow on river               
						 */
  int                              river_speed;		/*
							 * speed of flow on river             
							 */
  ...
  int                              tele_time;	/*
						 ...
						 */
  int                              tele_targ;	/*
						 * target room of a teleport          
						 */
  char                             tele_look;	/*
						 * do a do_look or not when 
						 * teleported                         
						 */
  ...
  char                            *sound;	/*
						 * sound made in this room            
						 */
  char                            *distant_sound;	/*
							 * sound made in ajoinging room       
							 */
...
#define AFF_SILENCED          0x00000040	/*
						 * this may have to go 
						 */
 ...
 */
#define ACT_SPEC       (1<<0)	/*
				 * special routine to be called if exist   
				 */
#define ACT_SENTINEL   (1<<1)	/*
				 * this mobile not to be moved             
				 */
#define ACT_SCAVENGER  (1<<2)	/*
				 * pick up stuff lying around              
				 */
#define ACT_ISNPC      (1<<3)	/*
				 * This bit is set for use with IS_NPC()   
				 */
#define ACT_NICE_THIEF (1<<4)	/*
				 * Set if a thief should NOT be killed     
				 */
#define ACT_AGGRESSIVE (1<<5)	/*
				 * Set if automatic attack on NPC's        
				 */
#define ACT_STAY_ZONE  (1<<6)	/*
				 * MOB Must stay inside its own zone       
				 */
#define ACT_WIMPY      (1<<7)	/*
				 * MOB Will flee when injured, and if      
				 */
			       /*
			        * aggressive only attack sleeping players 
			        */
#define ACT_ANNOYING   (1<<8)	/*
				 * MOB is so utterly irritating that other 
				 */
			       /*
			        * monsters will attack it...              
			        */
#define ACT_HATEFUL    (1<<9)	/*
				 * MOB will attack a PC or NPC matching a  
				 */
			       /*
			        * specified name                          
			        */
#define ACT_AFRAID    (1<<10)	/*
				 * MOB is afraid of a certain PC or NPC,   
				 */
			       /*
			        * and will always run away ....           
			        */
#define ACT_IMMORTAL  (1<<11)	/*
				 * MOB is a natural event, can't be kiled  
				 */
#define ACT_HUNTING   (1<<12)	/*
				 * MOB is hunting someone                  
				 */
#define ACT_DEADLY    (1<<13)	/*
				 * MOB has deadly poison                   
				 */
#define ACT_POLYSELF  (1<<14)	/*
				 * MOB is a polymorphed person             
				 */
#define ACT_POLYOTHER (1<<15)	/*
				 * MOB is a polymorphed person             
				 */
#define ACT_GUARDIAN  (1<<16)	/*
				 * MOB will guard master                   
				 */
#define ACT_USE_ITEM  (1<<17)	/*
				 * Will use an item when picked up         
				 */
#define ACT_FIGHTER_MOVES (1<<18)	/*
					 * mob will attack like a fighter 
					 */
#define ACT_FOOD_PROVIDE (1<<19)
#define ACT_PROTECTOR (1<<20)	/*
				 * mob will come to aid those that call 
				 */
				 /*
				  * this is inter zonal only             
				  */
#define ACT_MOUNT     (1<<21)	/*
				 * mobile can act as a mount      
				 */
...
#define PLR_DONTSET   (1<<3)	/*
				 * Dont EVER set 
				 */
#define PLR_WIMPY     (1<<4)	/*
				 * character will flee when seriously injured 
				 */
#define PLR_NOHASSLE  (1<<5)	/*
				 * char won't be attacked by aggressives.      
				 */
#define PLR_STEALTH   (1<<6)	/*
				 * char won't be seen in a variety of situations 
				 */
#define PLR_HUNTING   (1<<7)	/*
				 * the player is hunt somene,doa track each look 
				 */
#define PLR_DEAF      (1<<8)	/*
				 * The player does not hear shouts 
				 */
#define PLR_ECHO      (1<<9)	/*
				 * Messages (tells, shout,etc) echo back 
				 */
#define PLR_NOSHOUT   (1<<14)	/*
				 * the player is not allowed to shout 
				 */
#define PLR_PAGER     (1<<15)	/*
				 * Use the Game pager or not 
				 */
#define PLR_LOGS      (1<<16)	/*
				 * Player is writing tothe board 
				 */
...
#define NEW_PLR_KILLOK (1<<0)	/*
				 * players can kill each other 
				 */
#define NEW_PLR_NOTELL (1<<1)	/*
				 * players can't recieve tells 
				 */
#define NEW_PLR_SUMMON (1<<2)	/*
				 * This player is a pc killer 
				 */
#define NEW_PLR_TELEPORT (1<<3)	/*
				 * This player is a pc thief 
				 */

#define PRIV_GIVE_PRIV  (1<<0)	/*
				 * can this player give privs 
				 */
#define PRIV_FORCE	(1<<1)
#define PRIV_TRANS	(1<<2)	/*
				 * can the player use trans 
				 */
#define PRIV_SHUTDOWN	(1<<3)
#define PRIV_LOAD_PURGE	(1<<4)	/*
				 * can the playe load-purge mobs/objects 
				 */
#define PRIV_RESTORE	(1<<5)	/*
				 * can the player Heal to full 
				 */
#define PRIV_RESTORE_A  (1<<6)	/*
				 * can the player heal everyone to full 
				 */
#define PRIV_STAT	(1<<7)	/*
				 * can you stat anything in the game 
				 */
#define PRIV_SWITCH	(1<<8)
#define PRIV_REROLL_ADVANCE_PSET (1<<9)		/*
						 * can the player use the commands
...
#define PRIV_BUILDER	(1<<12)	/*
				 * This priv will allow the player to use
...
struct char_player_data {
  ...
  char                            *short_descr;		/*
							 ...
							 */
  char                            *long_descr;	/*
						 * for 'look'.. Only here for testing   
						 */
  char                            *description;		/*
							 * Extra descriptions                   
							 */
  ...
  char                            *guild_name;
  char                            *sounds;	/*
						 * Sound that the monster makes (in room) 
						 */
  char                            *distant_snds;	/*
							 * Sound that the monster makes (other) 
							 */
  byte                             sex;		/*
						 * PC / NPC s sex                       
						 */
  byte                             class;	/*
						 * PC s class or NPC alignment          
						 */
  byte                             level[6];	/*
						 * PC / NPC s level                     
						 */
...
struct char_point_data {
  ...
  int                              i4;
  int                              wiz_priv;	/*
						 * what kind of wiz privs they get 
						 */
...
struct char_special_data {
  ...
  int                              timer;	/*
						 ...
						 */
  int                              was_in_room;		/*
							 * storage of location for linkdead people 
							 */
  ...
  byte                             damsizedice;		/*
							 ...
							 */
  byte                             last_direction;	/*
							 * The last direction the monster went    
							 */
  int                              attack_type;		/*
...
struct char_skill_data {
  byte                             learned;	/*
						 * % chance for success 0 = not learned   
						 */
  bool                             recognise;	/*
						 * If you can recognise the scroll etc.   
						 */
};
 ...
 */
  ...
  sh_int                           fallspeed;	/*
						 * rate of descent for player 
						 */
  sh_int                           race;
  sh_int                           hunt_dist;	/*
						 * max dist the player can hunt 
						 */
  ...
  int                              act_ptr;	/*
						 * numeric argument for the mobile actions 
						 */
  ...
  struct char_ability_data         tmpabilities;	/*
							 * The abilities we will use   
							 */
  struct char_point_data           points;	/*
						 * Points                      
						 */
  struct char_special_data         specials;	/*
						 * Special plaing constants    
						 */
  ...
  struct descriptor_data          *desc;	/*
						 * NULL for mobiles            
						 */
  struct char_data                *orig;	/*
						 * Special for polymorph       
						 */
...
struct obj_cost {		/*
				 * used in act.other.c:do_save as
...
#if 0
#define MAX_OBJ_SAVE 99		/*
				 * Used in OBJ_FILE_U *DO*NOT*CHANGE* 
				 */
...
struct obj_file_u {
  char                             owner[20];	/*
						 ...
						 */
  int                              gold_left;	/*
						 * Number of goldcoins left at owner  
						 */
  int                              total_cost;	/*
						 * The cost for all items, per day    
						 */
  long                             last_update;		/*
							 * Time in seconds, when last updated 
							 */
...
struct descriptor_data {
  ...
  char                             last_input[MAX_INPUT_LENGTH];	/*
									 ...
									 */
  struct txt_q                     output;	/*
						 * q of strings to send       
						 */
  struct txt_q                     input;	/*
						 * q of unprocessed input     
						 */
...
struct message_list {
  ...
  int                              number_of_attacks;	/*
							 * How many attack messages to chose from. 
							 */
...
struct wis_app_type {
  byte                             bonus;	/*
						 * how many bonus skills a player can 
						 */
  /*
   * practice pr. level                 
   */
