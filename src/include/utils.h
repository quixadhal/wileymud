...
#define GOLD_NEEDED(ch, cl)                                                                                            \
    (IS_PC(ch) ? ((3 * (16 << ((int)GET_LEVEL(ch, cl) + 1) / 3) +                                                      \
                   ((16 << ((int)GET_LEVEL(ch, cl) + 1) / 3) * (((int)GET_LEVEL(ch, cl) + 1) % 3))) /                  \
                  (((int)GET_LEVEL(ch, cl) < 8) ? (((int)GET_LEVEL(ch, cl) < 4) ? 8 : 2) : 1))                         \
               : 0)
...
#define IS_LIGHTOUT(room)                                                                                              \
    ((!IS_SET(real_roomp(room)->room_flags, INDOORS) &&                                                                \
      ((weather_info.sunlight > SUN_DARK) || (FULL_MOON && !STORMY && !NO_MOON))) ||                                   \
     real_roomp(room)->light)
#define IS_DARKOUT(room)                                                                                               \
    ((!IS_SET(real_roomp(room)->room_flags, INDOORS) &&                                                                \
      ((weather_info.sunlight == SUN_DARK) && !(FULL_MOON && !STORMY && !NO_MOON))) &&                                 \
     !(real_roomp(room)->light))
