    ...
    case WHOD_OPEN:
        ...
        if (FD_ISSET(whod.socket, &in))
        {
            ...
            else
            {
                ...
                log_info("WHO request from %lu.%lu.%lu.%lu served.", (hostlong & 0xff000000) >> 24,
                         (hostlong & 0x00ff0000) >> 16, (hostlong & 0x0000ff00) >> 8, (hostlong & 0x000000ff) >> 0);
            }
...
char *whod_text(struct char_data *viewer)
{
    ...
    for (ch = character_list; ch; ch = ch->next)
    {
        ...
        if ((INVIS_LEVEL(ch) < 2) && (GetMaxLevel(ch) <= WIZ_MAX_LEVEL) && !IS_AFFECTED(ch, AFF_HIDE) &&
